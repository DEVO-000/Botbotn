...
# === Магазин кастомных товаров (платные кнопки/услуги) ===
# Структура: { "<product_id>": {name, description, price_xtr, price_virt,
#               grant_type, grant_value, active, created_at} }
PRODUCTS_FILE = _data_file("products.json")
...
EDIT_BELL_END = 112
# === Магазин кастомных товаров (платные кнопки/услуги) ===
DEV_PRODUCT_NAME = 113        # Название товара
DEV_PRODUCT_DESC = 114        # Описание товара
DEV_PRODUCT_PRICE_XTR = 115   # Цена в Telegram Stars (XTR)
DEV_PRODUCT_PRICE_VIRT = 116  # Цена в виртуальных звёздах (0 = недоступно)
DEV_PRODUCT_GRANT = 117       # Тип выдаваемой услуги
DEV_PRODUCT_GRANT_VALUE = 118 # Значение услуги (сколько звёзд / название услуги)
DEV_PRODUCT_PERIOD = 119      # Период подписки (дней): 0 = разовый товар
...
def load_prices():
    default_prices = {
        ...
        # Сколько виртуальных звёзд списывается за ОДНО сообщение в DEVO+ai.
...
def get_stars_keyboard(user, show_back=True):
    ...
    keyboard = [
        ...
        # НОВОЕ: магазин кастомных товаров — динамические чеки Stars.
...
QUICK_COMMANDS = [
    ...
    "⭐ Звезды", "🛍 Магазин", "🤖 DEVO+ai", "🪄 Автоматизация",
...
def get_developer_keyboard():
    ...
    keyboard = [
        ...
        # НОВОЕ: управление магазином кастомных товаров.
        [InlineKeyboardButton("🛍 Магазин товаров", callback_data="dev_shop")],
...
def _ai_chat_system_prompt():
    """Системный промпт чата DEVO+ai.

    Требование: чат с ИИ на DeepSeek API должен вести ЛЮБУЮ тему и МГНОВЕННО
    переключаться между темами — пользователь может сначала говорить об одном,
    а следующим сообщением резко о другом; бот не должен «тупить» и тащить
    старую тему за собой.
    """
    return (
        "Ты — DEVO+ai, умный и дружелюбный ИИ-помощник школьного Telegram-бота "
        "DEVO+WORKS. Отвечай ТОЛЬКО на русском языке, независимо от языка запроса.\n\n"
        "ГЛАВНЫЕ ПРИНЦИПЫ:\n"
...
async def handle_ai_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # === Синхронизация цен: плата за генерацию DEVO+ai ===
    ...
    if not user_conversations[user_id]:
        user_conversations[user_id].append({
            "role": "system",
            "content": _ai_chat_system_prompt(),
        })
    elif (
        user_conversations[user_id][0].get("role") == "system"
        and str(user_conversations[user_id][0].get("content", "")).startswith(
            "Ты должен отвечать ТОЛЬКО на русском"
        )
    ):
        # Мягкая миграция: старый однострочный system заменяем на новый
        # промпт с правилами смены тем и точного понимания запросов.
        user_conversations[user_id][0]["content"] = _ai_chat_system_prompt()
    ...
    ai_response = None
    if not photo_data_url and DEEPSEEK_API_KEY:
...
async def _automation_open_section(update, context, user, action):
    ...
    if target is None:
        return (
            ...
            "«зайди в звезды», «открой магазин», «покажи меню» — или назовите "
...
def _automation_system_prompt(context_text, is_admin):
    ...
    return (
        ...
        '9) {"action":"create_timer","date":"ГГГГ-ММ-ДД","time":"ЧЧ:ММ","text":"<текст напоминания>"} — таймер/напоминание (доступно всем).\n'
        ...
        '16) {"action":"show_weather"} — показать погоду в городе пользователя.\n'
        '17) {"action":"open_section","section":"<раздел или название кнопки>"} — ОТКРЫТЬ раздел интерфейса бота (автоматизация «нажимает» кнопку за пользователя). Стандартные разделы: меню, дз, добавить дз, удалить дз, учителя, расписание, звонки, каникулы, таймер, анонимное сообщение, мои анонимные сообщения, классы, мои кнопки, звезды, магазин, ии (чат с AI), погода, инструкция, код класса, написать админу, настройки. ТАКЖЕ можно открыть любую ЛИЧНУЮ, ГЛОБАЛЬНУЮ или КЛАССНУЮ кнопку, передав её точное название в section. Используй, когда пользователь просит открыть/зайти/показать раздел или кнопку («открой дз», «зайди в магазин», «включи чат с ии»). Не используй, если пользователь просит ДАННЫЕ (ДЗ/учителей) — для этого есть show_*.\n'
        '18) {"action":"show_vip"} — показать активные VIP-подписки пользователя и даты их окончания.\n'
        '19) {"action":"clarify","question":"<один конкретный вопрос>"} — если данных не хватает или запрос неоднозначен.\n'
        '20) {"action":"none","answer":"<ответ на общий вопрос>"} — запрос не про школьные действия: просто ответь по-русски.\n\n'
        "ПРАВИЛА:\n"
        ...
        "- ПОНИМАЙ СИНОНИМЫ ТОЧНО: «домашка»=ДЗ, «звонки»=время уроков, «училки/преподы»=учителя, «купить/магазин»=open_section магазин, «параграф»=задание. Название предмета сопоставляй с существующим по смыслу («русский»→«Русский язык», «матеша»→«Математика»).\n"
...
async def _automation_execute_action(update, context, user, class_obj, action):
    ...
    if name == "show_vip":
        ...
        if len(lines) == 1:
            return ("👑 Активных VIP-подписок нет.\n"
                    "Посмотреть, что можно купить: «🛍 Магазин» в главном меню "
                    "(или скажите «открой магазин»).", True)
        return "\n".join(lines), True
    ...
    if name in {
        "add_homework", "delete_homework", "replace_teachers", "add_subject",
        "remove_subject", "edit_schedule", "edit_bell", "set_holidays",
        "send_class_message",
    }:
        if not class_obj:
...
def _automation_help_text():
    return (
        "🪄 **Автоматизация**\n\n"
        "Напишите одним сообщением, что сделать — я пойму и выполню. Примеры:\n\n"
        "• «Русский на пятницу этой недели страница 45 упражнение 12»\n"
        "• «Математика на завтра задачи 10-15»\n"
        ...
        "• «Напомни завтра в 18:00 про секцию»\n"
        "• «Какое ДЗ на понедельник?» / «Покажи учителей»\n"
        "• «Открой магазин» / «Зайди в дз» / «Покажи меню» — открою любой раздел\n"
...
# ==================================
# === 🛍 МАГАЗИН КАСТОМНЫХ ТОВАРОВ ===
# ==================================
# Разработчик создаёт любые «платные кнопки» (товары/услуги) со свободной
# ценой. Пользователь платит Telegram Stars (XTR) или виртуальными звёздами —
# бот динамически выставляет счёт под сумму товара и МГНОВЕННО зачисляет
# услугу после оплаты. Поддерживаются и внешние платёжные системы: если на
# сервере задан PAYMENT_PROVIDER_TOKEN, товар можно купить в фиате
# (валюта PAYMENT_CURRENCY).

PRODUCT_GRANT_TYPES = {
    "stars": "⭐ Начислить виртуальные звёзды",
    "button_slot": "➕ +1 платная личная кнопка",
    "anon_month": "📨 +1 месяц места под анонимки",
    "vip": "👑 VIP-доступ (подписка на период)",
    "custom": "🧾 Кастомная услуга (ручная выдача)",
}

PRODUCT_PAY_PAYLOAD_PREFIX = "buy_prod_"
...
def save_products(products):
    return save_data(PRODUCTS_FILE, products)


def get_product(product_id):
    return load_products().get(str(product_id))


def _product_label_price(product):
    """Человекочитаемая цена товара для кнопок/списков."""
    parts = []
    xtr = int(product.get("price_xtr") or 0)
    virt = int(product.get("price_virt") or 0)
    if xtr > 0:
        parts.append(f"{xtr} XTR")
    if virt > 0:
        parts.append(f"{virt} ⭐")
    label = " / ".join(parts) if parts else "цена не задана"
    # Период подписки (если задан) — прямо в подписи кнопки.
    days = _product_period_days(product)
    if days > 0:
        label += f" · {days} дн."
    return label
...
# === 👑 VIP-ПОДПИСКИ С ПЕРИОДОМ ===
# ==================================
# Товар с period_days > 0 — это подписка: после оплаты она активна указанное
# число дней (месяц/неделя/N дней — как указал разработчик), и ПОКА ОНА
# АКТИВНА пользователь НЕ МОЖЕТ купить тот же товар повторно. Когда период
# истекает — кнопка покупки снова доступна. Разовые товары (period_days = 0)
# ведут себя как раньше: покупай сколько хочешь.

def _product_period_days(product):
    """Период подписки товара в днях (0 = разовый товар)."""
    try:
        days = int(product.get("period_days") or 0)
    except (TypeError, ValueError):
        days = 0
    if days < 0:
        days = 0
...
def _sub_key_for_product(product):
    """Ключ подписки в user.subscriptions.

    Для vip/custom берём имя услуги (grant_value/имя товара) — так одна и та
    же «VIP-функция» остаётся заблокированной даже после пересоздания товара.
    Для остальных — product_id.
    """
    grant_type = product.get("grant_type")
    if grant_type in ("vip", "custom"):
        name = str(product.get("grant_value") or product.get("name") or "").strip()
        if name:
            return f"vip:{name.lower()}"
    return f"prod:{product.get('product_id')}"
...
def _get_active_subscription(user, product):
    """Возвращает datetime окончания АКТИВНОЙ подписки пользователя на товар
    или None, если подписки нет либо она уже истекла."""
    if not user or not product:
        return None
    subs = getattr(user, "subscriptions", {}) or {}
    until = _parse_sub_until(subs.get(_sub_key_for_product(product)))
    if until is None:
        return None
...
def _grant_subscription(user, product):
    """Начисляет/продлевает подписку пользователя на период товара.

    Продление считается от МАКСИМУМА (сейчас, текущее окончание) — если
    пользователь somehow оплатил повторно в гонке, дни не «сгорают».
    Сохраняет пользователя и возвращает datetime нового окончания.
    """
    days = _product_period_days(product)
    if days <= 0:
        days = 30  # страховка: VIP-товар без периода живёт месяц
    subs = dict(getattr(user, "subscriptions", {}) or {})
    key = _sub_key_for_product(product)
    current = _parse_sub_until(subs.get(key))
    base = datetime.utcnow()
    if current is not None and current > base:
        base = current
    new_until = base + timedelta(days=days)
    display_name = str(
        product.get("grant_value") or product.get("name") or key
    ).strip() or key
    subs[key] = {"until": new_until.isoformat(timespec="seconds"), "name": display_name}
...
def _human_period(days):
    """Человекочитаемый период: 30 → «30 дней (месяц)», 7 → «7 дней (неделя)»."""
    if days <= 0:
        return "разовый товар"
    if days == 1:
        return "1 день"
    if days == 7:
        return "7 дней (неделя)"
    if days in (28, 30, 31):
        return f"{days} дней (месяц)"
    if days in (365, 366):
        return f"{days} дней (год)"
    return f"{days} дней"


def _subscription_block_text(user, product):
    """Текст о том, что товар уже активен (для магазина и precheckout)."""
    until = _get_active_subscription(user, product)
    days = _product_period_days(product)
    return (
        f"👑 У вас уже активна эта функция — {_human_period(days)} до "
        f"{_fmt_sub_until(until)}.\n\n"
        "Повторная покупка сейчас недоступна: она станет возможна после "
        "окончания периода."
    )


def _fulfill_product_purchase(user, product):
    """Мгновенно зачисляет услугу товара. Возвращает текст, что именно
    зачислено. Не выставляет счета и не списывает звёзды — только зачисление.

    VIP/подписочные товары (period_days > 0 или grant_type == "vip")
    зачисляются как подписка: активна N дней, повторная покупка до окончания
    периода заблокирована."""
    grant_type = product.get("grant_type") or "custom"
    grant_value = product.get("grant_value")

    # === Подписочные товары: VIP или любой товар с периодом ===
    if grant_type == "vip" or _product_period_days(product) > 0:
        new_until = _grant_subscription(user, product)
        days = _product_period_days(product)
        return (
            f"👑 Функция активирована ({_human_period(days)})!\n"
            f"📅 Действует до: {_fmt_sub_until(new_until)}.\n"
            "Повторная покупка станет доступна после окончания периода."
        )

    if grant_type == "stars":
        try:
            amount = int(grant_value or 0)
        except (TypeError, ValueError):
            amount = 0
        if amount > 0:
            add_stars_transaction(user.user_id, amount, f"Товар «{product.get('name')}»")
            return f"⭐ Зачислено {amount} виртуальных звёзд."
        return "⚠️ Товар настроен без суммы зачисления — сообщите разработчику."

    if grant_type == "button_slot":
        user.prepaid_buttons = int(getattr(user, "prepaid_buttons", 0) or 0) + 1
        save_user(user)
        return "➕ +1 платная личная кнопка: можно создать кнопку без доплаты (🌟 Мои кнопки)."

    if grant_type == "anon_month":
        new_until = _extend_user_anon_keep(user, months=1)
        return (
            "📨 +1 месяц места под анонимные сообщения: авто-очистка их не тронет "
            f"до {new_until.strftime('%Y-%m-%d %H:%M')} UTC."
        )

    # custom — кастомная услуга: счётчик + уведомление разработчику.
    service_name = str(grant_value or product.get("name") or "Услуга").strip()
    services = getattr(user, "services", {}) or {}
    services[service_name] = int(services.get(service_name, 0) or 0) + 1
    user.services = services
    save_user(user)
    return (
        f"🧾 Услуга «{service_name}» зачислена (всего: {services[service_name]}). "
        "Разработчик получил уведомление и выполнит её."
    )


async def _notify_dev_product_sold(context, user, product, payment_desc):
    """Уведомляет разработчика о продаже товара (для кастомных услуг —
    это сигнал к ручной выдаче)."""
    try:
        if not DEVELOPER_ID:
            return
        await context.bot.send_message(
            ...
            text=(
                f"🛍 Продажа товара: «{product.get('name')}»\n"
                f"👤 Пользователь: {user.first_name} (ID {user.user_id})\n"
                f"💳 Оплата: {payment_desc}\n"
                f"📦 Услуга: {PRODUCT_GRANT_TYPES.get(product.get('grant_type'), product.get('grant_type'))}"
                + (f" → {product.get('grant_value')}" if product.get("grant_value") else "")
            ),
        )
    except Exception as e:
        logger.warning(f"notify dev product sold: {e}")


def get_shop_keyboard(user):
    """Список активных товаров магазина. У товаров с активной подпиской
    показывается бейдж «👑 активна до …»."""
    products = [p for p in load_products().values() if p.get("active", True)]
    keyboard = []
    for product in sorted(products, key=lambda p: str(p.get("created_at", ""))):
        pid = str(product.get("product_id"))
        label = f"{product.get('name', 'Товар')} — {_product_label_price(product)}"
        active_until = _get_active_subscription(user, product)
        if active_until is not None:
            label = f"👑 {product.get('name', 'Товар')} — активна до {_fmt_sub_until(active_until)}"
        keyboard.append([InlineKeyboardButton(
            label,
            callback_data=f"shop_prod_{pid}",
        )])
    keyboard.append([InlineKeyboardButton("⬅️ Назад", callback_data="back_to_main")])
    return InlineKeyboardMarkup(keyboard)
...
async def shop_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Меню «🛍 Магазин» — работает и из главного меню, и из колбэка."""
    query = getattr(update, "callback_query", None)
    if query is not None:
        user_id = str(query.from_user.id)
        user = get_user(user_id) or User(user_id)
        text = "🛍 **Магазин**\n\nВыберите товар — счёт придёт автоматически:"
        try:
            await query.edit_message_text(
                text, reply_markup=get_shop_keyboard(user), parse_mode="Markdown"
            )
        except Exception:
            await context.bot.send_message(
                chat_id=int(user_id),
    ...
    # Вызов кнопкой ReplyKeyboard из главного меню.
    user_id = str(update.effective_user.id)
    user = get_user(user_id) or User(user_id)
    await update.message.reply_text(
        "🛍 **Магазин**\n\nВыберите товар — счёт придёт автоматически:",
...
async def shop_product_view_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Карточка товара: описание + кнопки оплаты (XTR / виртуальные звёзды)."""
    query = update.callback_query
    await query.answer()
    user_id = str(query.from_user.id)
    user = get_user(user_id) or User(user_id)

    pid = query.data[len("shop_prod_"):]
    ...
    xtr = int(product.get("price_xtr") or 0)
    virt = int(product.get("price_virt") or 0)
    grant_desc = PRODUCT_GRANT_TYPES.get(product.get("grant_type"), "Услуга")
    if product.get("grant_value"):
        grant_desc += f" → {product.get('grant_value')}"
    days = _product_period_days(product)
    if days > 0:
        grant_desc += f" (период: {_human_period(days)})"

    # 👑 VIP-подписка уже активна — повторная покупка запрещена до конца периода.
    active_until = _get_active_subscription(user, product)
    if active_until is not None:
        await query.edit_message_text(
            f"🛍 **{product.get('name')}**\n\n"
            f"{product.get('description') or ''}\n\n"
            f"{_subscription_block_text(user, product)}",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🛍 К другим товарам", callback_data="shop_menu")],
            ]),
            parse_mode="Markdown",
        )
        return MAIN_MENU
    ...
    text = (
        f"🛍 **{product.get('name')}**\n\n"
        f"{product.get('description') or 'Описание не указано.'}\n\n"
        f"💳 Цена: {_product_label_price(product)}\n"
        f"📦 Что получите: {grant_desc}\n"
        f"💰 Ваш баланс: {user.stars_balance} ⭐"
    )
    keyboard = []
    if xtr > 0:
        keyboard.append([InlineKeyboardButton(f"💎 Оплатить {xtr} XTR (Telegram Stars)", callback_data=f"shop_xtr_{pid}")])
    if virt > 0:
        keyboard.append([InlineKeyboardButton(f"⭐ Оплатить {virt} виртуальными звёздами", callback_data=f"shop_virt_{pid}")])
    keyboard.append([InlineKeyboardButton("⬅️ В магазин", callback_data="shop_menu")])
    await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
    return MAIN_MENU


async def shop_buy_virt_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Оплата товара виртуальными звёздами — моментальное зачисление."""
    query = update.callback_query
    await query.answer()
    user_id = str(query.from_user.id)
    user = get_user(user_id) or User(user_id)

    pid = query.data[len("shop_virt_"):]
    product = get_product(pid)
    if not product or not product.get("active", True):
    ...
    virt = int(product.get("price_virt") or 0)
    if virt <= 0:
        await query.edit_message_text("Этот товар нельзя купить виртуальными звёздами.")
        return MAIN_MENU

    # 👑 Повторная покупка активной подписки запрещена.
    if _get_active_subscription(user, product) is not None:
        await query.edit_message_text(
            _subscription_block_text(user, product),
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ В магазин", callback_data="shop_menu")]]),
        )
        return MAIN_MENU

    if user.stars_balance < virt:
        await query.edit_message_text(
            f"❌ Недостаточно звёзд.\n\n💰 Баланс: {user.stars_balance} ⭐\n💎 Нужно: {virt} ⭐",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("💎 Купить звёзды", callback_data="buy_stars")],
                [InlineKeyboardButton("⬅️ В магазин", callback_data="shop_menu")],
            ]),
        )
        return MAIN_MENU

    # Списываем и зачисляем услугу.
    add_stars_transaction(user_id, -virt, f"Товар «{product.get('name')}»")
    user = get_user(user_id) or user  # свежая копия после списания
    granted_text = _fulfill_product_purchase(user, product)
    await _notify_dev_product_sold(context, user, product, f"{virt} виртуальных ⭐")
    await query.edit_message_text(
        f"✅ Оплата прошла: −{virt} ⭐\n\n{granted_text}",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ В магазин", callback_data="shop_menu")]]),
    )
    return MAIN_MENU


async def shop_buy_xtr_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Динамический счёт Telegram Stars (или внешней платёжной системы)
    ровно на сумму товара."""
    query = update.callback_query
    await query.answer()
    user_id = str(query.from_user.id)

    pid = query.data[len("shop_xtr_"):]
    product = get_product(pid)
    if not product or not product.get("active", True):
        await query.edit_message_text("Товар не найден или снят с продажи.")
        return MAIN_MENU
    ...
    xtr = int(product.get("price_xtr") or 0)
    if xtr <= 0:
        await query.edit_message_text("Этот товар нельзя купить за Telegram Stars.")
        return MAIN_MENU

    # 👑 Повторная покупка активной подписки запрещена — счёт не выставляем.
    user_obj = get_user(user_id) or User(user_id)
    if _get_active_subscription(user_obj, product) is not None:
        await query.edit_message_text(
            _subscription_block_text(user_obj, product),
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ В магазин", callback_data="shop_menu")]]),
        )
        return MAIN_MENU

    # Помним, какой товар оплащается — successful_payment по payload
    # buy_prod_<pid> сам найдёт товар, а pending_payment страхует от
    # правок цены между выставлением счёта и оплатой.
    pending_payments[f"prod_{user_id}"] = pid

    if PAYMENT_PROVIDER_TOKEN:
        # Внешняя платёжная система (провайдер Telegram Payments): фиат.
        # Цена берётся из поля price_fiat (в МИНИМАЛЬНЫХ единицах валюты:
        # копейки/центы). Если поле не задано — fallback xtr*100.
        await context.bot.send_invoice(
            chat_id=user_id,
            title=str(product.get("name"))[:32],
            description=str(product.get("description") or product.get("name"))[:255],
            payload=f"{PRODUCT_PAY_PAYLOAD_PREFIX}{pid}",
            provider_token=PAYMENT_PROVIDER_TOKEN,
            currency=PAYMENT_CURRENCY,
            prices=[LabeledPrice(
                label=str(product.get("name"))[:32],
                amount=int(product.get("price_fiat") or xtr * 100),
            )],
            start_parameter=f"product-{pid}",
        )
    else:
        # Telegram Stars (XTR): сумма ровно как у товара.
        await context.bot.send_invoice(
            chat_id=user_id,
            title=str(product.get("name"))[:32],
            description=str(product.get("description") or product.get("name"))[:255],
            ...
            provider_token="",
            currency="XTR",
            prices=[LabeledPrice(label=str(product.get("name"))[:32], amount=xtr)],
            start_parameter=f"product-{pid}",
        )
    try:
        await query.edit_message_text("💳 Счёт отправлен. После оплаты услуга зачислится автоматически.")
    except Exception:
        pass
    return MAIN_MENU


# --- Панель разработчика: управление товарами ---

async def dev_shop_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    user_id = str(query.from_user.id)
    ...
    keyboard = [[InlineKeyboardButton("➕ Создать товар", callback_data="dev_shop_create")]]
    for pid, product in sorted(products.items()):
        status = "🟢" if product.get("active", True) else "🔴"
        keyboard.append([InlineKeyboardButton(
            f"{status} {product.get('name', 'Товар')} — {_product_label_price(product)}",
            callback_data=f"dev_shop_view_{pid}",
        )])
    keyboard.append([InlineKeyboardButton("⬅️ Назад", callback_data="dev_back_panel")])
    await query.edit_message_text(
        f"🛍 **Магазин товаров** (всего: {len(products)})\n\n"
        "Товары покупаются через «🛍 Магазин» в ⭐ Звёздах и главном меню. "
        "Счёт выставляется динамически под цену товара, услуга зачисляется мгновенно.",
...
async def dev_shop_view_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Карточка товара в dev-панели: включить/выключить, удалить."""
    query = update.callback_query
    await query.answer()
    user_id = str(query.from_user.id)
    if user_id != DEVELOPER_ID:
    ...
    pid = query.data[len("dev_shop_view_"):]
    product = get_product(pid)
    if not product:
        await query.edit_message_text("Товар не найден.")
        return await dev_shop_start(update, context)

    status = "🟢 активен" if product.get("active", True) else "🔴 выключен"
    toggle_label = "🔴 Выключить" if product.get("active", True) else "🟢 Включить"
    days = _product_period_days(product)
    period_line = (
        f"⏳ Период подписки: {_human_period(days)}"
        + (" — повторная покупка блокируется пока активна" if days > 0 else "") + "\n"
        if product.get("grant_type") in ("vip", "custom") else ""
    )
    text = (
        f"🛍 **{product.get('name')}**\n\n"
        f"{product.get('description') or 'Без описания.'}\n\n"
        f"💎 Цена XTR: {product.get('price_xtr', 0)}\n"
        f"⭐ Цена виртуальными: {product.get('price_virt', 0)}\n"
        f"💳 Цена фиатом: {product.get('price_fiat', '—')} (мин. ед.)\n"
        f"📦 Услуга: {PRODUCT_GRANT_TYPES.get(product.get('grant_type'), '?')}"
        + (f" → {product.get('grant_value')}" if product.get('grant_value') else "") + "\n"
        + period_line +
        f"Статус: {status}"
    )
    keyboard = [
        [InlineKeyboardButton(toggle_label, callback_data=f"dev_shop_toggle_{pid}")],
        [InlineKeyboardButton("🗑️ Удалить товар", callback_data=f"dev_shop_del_{pid}")],
        [InlineKeyboardButton("⬅️ К товарам", callback_data="dev_shop")],
    ]
    await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
    return DEV_PANEL


async def dev_shop_toggle_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    if str(query.from_user.id) != DEVELOPER_ID:
        await query.edit_message_text("Доступ запрещён.")
        return DEV_PANEL
    pid = query.data[len("dev_shop_toggle_"):]
    products = load_products()
    if pid in products:
        products[pid]["active"] = not products[pid].get("active", True)
        save_products(products)
    return await dev_shop_view_handler(update, context)


async def dev_shop_delete_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    if str(query.from_user.id) != DEVELOPER_ID:
        await query.edit_message_text("Доступ запрещён.")
        return DEV_PANEL
    pid = query.data[len("dev_shop_del_"):]
    products = load_products()
    if pid in products:
        deleted_name = products[pid].get("name", pid)
        del products[pid]
        save_products(products)
        await query.edit_message_text(f"🗑️ Товар «{deleted_name}» удалён.")
        return await dev_shop_start(update, context)
    await query.edit_message_text("Товар не найден.")
    return await dev_shop_start(update, context)


async def dev_shop_create_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Пошаговое создание товара: имя → описание → XTR → виртуальные → услуга → значение."""
    query = update.callback_query
    await query.answer()
    if str(query.from_user.id) != DEVELOPER_ID:
        await query.edit_message_text("Доступ запрещён.")
    ...
    await query.edit_message_text(
        "➕ **Создание товара — шаг 1**\n\nВведите НАЗВАНИЕ товара\n(например: «VIP-статус» или «+100 звёзд»):",
        reply_markup=get_cancel_keyboard(),
        parse_mode="Markdown",
    )
    return DEV_PRODUCT_NAME


@timeout(CONVERSATION_TIMEOUT)
async def dev_product_name_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if str(update.effective_user.id) != DEVELOPER_ID:
        await update.message.reply_text("Доступ запрещён.")
        return await developer_panel(update, context)
    text_in = update.message.text.strip()
    rejected = await reject_if_forbidden_chars(update, text_in, DEV_PRODUCT_NAME)
    if rejected is not None:
        return rejected
    context.user_data.setdefault('new_product', {})['name'] = text_in[:64]
    await update.message.reply_text(
        f"Шаг 2 — описание товара «{text_in[:64]}»\n\nВведите описание (что покупатель получит):",
        reply_markup=get_cancel_keyboard(),
    )
    return DEV_PRODUCT_DESC


@timeout(CONVERSATION_TIMEOUT)
async def dev_product_desc_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if str(update.effective_user.id) != DEVELOPER_ID:
        await update.message.reply_text("Доступ запрещён.")
        return await developer_panel(update, context)
    text_in = update.message.text.strip()
    rejected = await reject_if_forbidden_chars(update, text_in, DEV_PRODUCT_DESC)
    if rejected is not None:
        return rejected
    context.user_data['new_product']['description'] = text_in[:512]
    await update.message.reply_text(
        "Шаг 3 — цена в Telegram Stars (XTR)\n\nВведите ЦЕЛОЕ ЧИСЛО (например 50).\n"
        "0 — товар нельзя купить за Stars:",
        reply_markup=get_cancel_keyboard(),
    )
    return DEV_PRODUCT_PRICE_XTR


@timeout(CONVERSATION_TIMEOUT)
async def dev_product_price_xtr_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if str(update.effective_user.id) != DEVELOPER_ID:
        await update.message.reply_text("Доступ запрещён.")
        return await developer_panel(update, context)
    try:
        ...
        await update.message.reply_text(
            "Введите целое число от 0 до 2500 (Telegram Stars):",
            reply_markup=get_cancel_keyboard(),
        )
        return DEV_PRODUCT_PRICE_XTR
    context.user_data['new_product']['price_xtr'] = value
    await update.message.reply_text(
        "Шаг 4 — цена в ВИРТУАЛЬНЫХ звёздах\n\nВведите целое число (0 — нельзя купить виртуальными):",
        reply_markup=get_cancel_keyboard(),
    )
    return DEV_PRODUCT_PRICE_VIRT


@timeout(CONVERSATION_TIMEOUT)
async def dev_product_price_virt_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if str(update.effective_user.id) != DEVELOPER_ID:
        await update.message.reply_text("Доступ запрещён.")
        return await developer_panel(update, context)
    try:
    ...
    product['price_virt'] = value
    if int(product.get('price_xtr', 0)) == 0 and value == 0:
        await update.message.reply_text(
            "⚠️ Обе цены нулевые. Задайте хотя бы одну цену — повторите цену виртуальными звёздами:",
            reply_markup=get_cancel_keyboard(),
        )
        return DEV_PRODUCT_PRICE_VIRT
    context.user_data['new_product'] = product
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton(PRODUCT_GRANT_TYPES["stars"], callback_data="dev_prod_grant_stars")],
        [InlineKeyboardButton(PRODUCT_GRANT_TYPES["button_slot"], callback_data="dev_prod_grant_button_slot")],
        [InlineKeyboardButton(PRODUCT_GRANT_TYPES["anon_month"], callback_data="dev_prod_grant_anon_month")],
        [InlineKeyboardButton(PRODUCT_GRANT_TYPES["vip"], callback_data="dev_prod_grant_vip")],
        [InlineKeyboardButton(PRODUCT_GRANT_TYPES["custom"], callback_data="dev_prod_grant_custom")],
    ])
    await update.message.reply_text(
        "Шаг 5 — какую услугу зачислять после оплаты?\n\n"
        "👑 VIP-доступ — подписка на период: пока активна, повторно купить нельзя.",
        reply_markup=kb,
    )
    return DEV_PRODUCT_GRANT


async def dev_product_grant_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    if str(query.from_user.id) != DEVELOPER_ID:
        await query.edit_message_text("Доступ запрещён.")
        return DEV_PANEL
    grant_type = query.data.replace("dev_prod_grant_", "")
    if grant_type not in PRODUCT_GRANT_TYPES:
        await query.edit_message_text("Неизвестный тип услуги.")
        return await dev_shop_start(update, context)
    product = context.user_data.get('new_product') or {}
    product['grant_type'] = grant_type
    context.user_data['new_product'] = product

    if grant_type == "button_slot":
        # Значение фиксировано — сразу сохраняем товар.
        product['grant_value'] = 1
        product.setdefault('period_days', 0)
        return await _save_new_product_and_confirm(update, context)
    if grant_type == "anon_month":
        product['grant_value'] = 1
        product.setdefault('period_days', 0)
        return await _save_new_product_and_confirm(update, context)
    if grant_type == "stars":
        await query.edit_message_text(
            "Шаг 6 — сколько ВИРТУАЛЬНЫХ звёзд зачислять после оплаты? Введите целое число:",
            reply_markup=get_cancel_keyboard(),
        )
        return DEV_PRODUCT_GRANT_VALUE
    if grant_type == "vip":
        # VIP-функция: сначала название функции, затем обязательный период.
        await query.edit_message_text(
            "Шаг 6 — назовите VIP-ФУНКЦИЮ (например «VIP-статус», «Безлимитный ИИ»):\n\n"
            "Пока подписка активна, пользователь НЕ сможет купить её повторно.",
            reply_markup=get_cancel_keyboard(),
        )
        return DEV_PRODUCT_GRANT_VALUE
    # custom — название услуги, затем период (0 = разовая).
    await query.edit_message_text(
        "Шаг 6 — назовите услугу (например «консультация», «разбор ДЗ»):",
        reply_markup=get_cancel_keyboard(),
    )
    return DEV_PRODUCT_GRANT_VALUE
...
@timeout(CONVERSATION_TIMEOUT)
async def dev_product_grant_value_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if str(update.effective_user.id) != DEVELOPER_ID:
        await update.message.reply_text("Доступ запрещён.")
        return await developer_panel(update, context)
    product = context.user_data.get('new_product') or {}
    text_in = update.message.text.strip()
    rejected = await reject_if_forbidden_chars(update, text_in, DEV_PRODUCT_GRANT_VALUE)
    if rejected is not None:
        return rejected
    if product.get('grant_type') == 'stars':
        try:
            value = int(text_in)
            if value <= 0:
                raise ValueError
            ...
            await update.message.reply_text(
                "Введите целое число больше 0:", reply_markup=get_cancel_keyboard()
            )
            return DEV_PRODUCT_GRANT_VALUE
        product['grant_value'] = value
        product.setdefault('period_days', 0)
        context.user_data['new_product'] = product
        return await _save_new_product_and_confirm(update, context)
    # vip / custom — название услуги/функции, затем шаг периода.
    product['grant_value'] = text_in[:64]
    context.user_data['new_product'] = product
    if product.get('grant_type') == 'vip':
        await update.message.reply_text(
            "Последний шаг — ПЕРИОД ПОДПИСКИ в днях.\n\n"
            "Введите целое число дней (например 7 — неделя, 30 — месяц).\n"
            "Пока подписка активна, купить её повторно будет НЕЛЬЗЯ:",
            reply_markup=get_cancel_keyboard(),
        )
        return DEV_PRODUCT_PERIOD
    await update.message.reply_text(
        "Последний шаг — период подписки в ДНЯХ.\n\n"
        "0 — разовая услуга (можно покупать сколько угодно раз).\n"
        "Например 30 — услуга-подписка на месяц: пока активна, повторно купить нельзя:",
        reply_markup=get_cancel_keyboard(),
    )
    return DEV_PRODUCT_PERIOD
...
@timeout(CONVERSATION_TIMEOUT)
async def dev_product_period_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Ввод периода подписки товара (в днях). 0 = разовый товар."""
    if str(update.effective_user.id) != DEVELOPER_ID:
        await update.message.reply_text("Доступ запрещён.")
        return await developer_panel(update, context)
    product = context.user_data.get('new_product') or {}
    try:
        ...
        await update.message.reply_text(
            "Введите целое число дней (0 — разовый товар, 7 — неделя, 30 — месяц):",
            reply_markup=get_cancel_keyboard(),
        )
        return DEV_PRODUCT_PERIOD
    if days < 0 or days > 3650:
        await update.message.reply_text(
            "Число дней должно быть от 0 до 3650:", reply_markup=get_cancel_keyboard()
        )
        return DEV_PRODUCT_PERIOD
    if product.get('grant_type') == 'vip' and days <= 0:
        # VIP без периода теряет смысл — честно требуем период > 0.
        await update.message.reply_text(
            "👑 VIP-функция обязана иметь период больше 0 (иначе её нельзя "
            "ограничить от повторной покупки). Введите число дней ≥ 1:",
            reply_markup=get_cancel_keyboard(),
        )
        return DEV_PRODUCT_PERIOD
...
async def _save_new_product_and_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Сохраняет товар из context.user_data['new_product'] и показывает итог.

    Может вызываться как из текстового шага (update.message), так и из
    колбэка выбора типа услуги (update.callback_query) — обрабатываем оба.
    """
    product = context.user_data.pop('new_product', None)
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("🛍 К списку товаров", callback_data="dev_shop")],
        [InlineKeyboardButton("⬅️ В панель разработчика", callback_data="dev_back_panel")],
    ])
    if not product:
        text = "Данные товара не найдены."
        keyboard = get_cancel_keyboard()
    else:
        import uuid as _uuid
        product_id = _uuid.uuid4().hex[:8]
        product['product_id'] = product_id
        product['active'] = True
        product['created_at'] = datetime.now().strftime("%Y-%m-%d %H:%M")
        products = load_products()
        products[product_id] = product
        save_products(products)
        grant_desc = PRODUCT_GRANT_TYPES.get(product.get('grant_type'), '?')
        if product.get('grant_value'):
            grant_desc += f" → {product.get('grant_value')}"
        days = _product_period_days(product)
        period_note = ""
        if product.get('grant_type') in ('vip', 'custom'):
            period_note = (
                f"⏳ Период: {_human_period(days)}"
                + (" — пока активна, повторно купить НЕЛЬЗЯ 👑" if days > 0 else "") + "\n"
            )
        text = (
            f"✅ Товар создан!\n\n"
            f"🛍 {product.get('name')}\n"
            f"{product.get('description') or ''}\n\n"
            f"💎 XTR: {product.get('price_xtr', 0)} | ⭐ виртуальные: {product.get('price_virt', 0)}\n"
            f"📦 Услуга: {grant_desc}\n"
            f"{period_note}\n"
            "Он уже доступен в «🛍 Магазине»."
        )
    if getattr(update, "callback_query", None) is not None:
        try:
            ...
            pass  # падение edit — отправим новым сообщением ниже
...
async def handle_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif message_text == "🛍 Магазин":
        # NEW: магазин кастомных товаров с динамическими счетами Stars.
        return await shop_menu_handler(update, context)
...
async def precheckout_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if payload.startswith("buy_stars_"):
        ...
        await query.answer(ok=True)
    elif payload.startswith(PRODUCT_PAY_PAYLOAD_PREFIX):
        # Товар из «🛍 Магазина» — динамический счёт под любую сумму.
        # Пропускаем только если товар существует, активен и у пользователя
        # НЕТ активной подписки на него (VIP нельзя купить дважды за период).
        _pid = payload[len(PRODUCT_PAY_PAYLOAD_PREFIX):]
        _product = get_product(_pid)
        _sub_user = get_user(str(query.from_user.id))
        if _product and _product.get("active", True) and _get_active_subscription(_sub_user, _product) is None:
            await query.answer(ok=True)
        elif _product and _get_active_subscription(_sub_user, _product) is not None:
            await query.answer(
                ok=False,
                error_message="Эта функция уже активна у вас до конца периода — повторная покупка недоступна.",
            )
        else:
            await query.answer(ok=False, error_message="Товар недоступен, счёт отменён")
    else:
...
async def successful_payment_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if payload.startswith(PRODUCT_PAY_PAYLOAD_PREFIX):
        # === 🛍 Магазин: автоматическая обработка платежа и мгновенное
        # зачисление услуги товара (payload вида buy_prod_<id>).
        pid = payload[len(PRODUCT_PAY_PAYLOAD_PREFIX):]
        product = get_product(pid)
        if not product:
            await update.message.reply_text(
                "⚠️ Оплата прошла, но товар не найден. Сообщите разработчику — "
                "он доначислит услугу вручную."
            )
            await _notify_dev_product_sold(context, user, {"name": f"(удалён) {pid}"}, f"{_paid_amount} XTR")
            return MAIN_MENU
        # Честная защита от гонки: если между выставлением счёта и оплатой
        # подписка уже активирована — продлеваем её от текущего окончания
        # (дни не сгорают, деньги пользователя не теряются).
        granted_text = _fulfill_product_purchase(user, product)
        await _notify_dev_product_sold(context, user, product, f"{_paid_amount} XTR")
        await update.message.reply_text(
...
async def dev_set_prices_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"• Генерация DEVO+ai: {PRICES.get('ai_generation', 0)} ⭐ (0 = бесплатно)\n\n"
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # === НОВОЕ: 🛍 Магазин кастомных товаров ===
    # ВАЖНО: ветка shop_xtr_ должна идти РАНЬШЕ общей проверки shop_, а
    # shop_prod_ — раньше shop_virt_/shop_xtr_ не пересекаются, но
    # startswith("shop_prod_") и startswith("shop_") — префиксы разные, так
    # что порядок безопасен.
    if data == "shop_menu":
        return await shop_menu_handler(update, context)
    elif data.startswith("shop_prod_"):
        return await shop_product_view_handler(update, context)
    elif data.startswith("shop_virt_"):
        return await shop_buy_virt_handler(update, context)
    elif data.startswith("shop_xtr_"):
        return await shop_buy_xtr_handler(update, context)

    # === НОВОЕ: dev-управление магазином и сброс статистики Stars ===
    if data == "dev_shop":
        return await dev_shop_start(update, context)
    elif data == "dev_shop_create":
        return await dev_shop_create_start(update, context)
    elif data.startswith("dev_shop_view_"):
        return await dev_shop_view_handler(update, context)
    elif data.startswith("dev_shop_toggle_"):
        return await dev_shop_toggle_handler(update, context)
    elif data.startswith("dev_shop_del_"):
        return await dev_shop_delete_handler(update, context)
    elif data.startswith("dev_prod_grant_"):
        return await dev_product_grant_handler(update, context)
    elif data == "dev_reset_stars":
...
async def detect_timezone_for_city(city):
    ...
    ИСПРАВЛЕНО (баг часового пояса погоды): раньше смещение вычислялось из
    строкового `location.localtime` с точностью до МИНУТ, из-за чего смещения
    типа UTC+5:30 (Индия) округлялись неправильно, а при drift секунд
    сервера получался лишний ±1 час. Теперь:
      1) WeatherAPI `location.localtime_epoch` — точное смещение по секундам,
         с поддержкой дробных поясов (округляем до ближайшей ПОЛОВИНЫ часа);
    ...
    Возвращаемое значение — число (int для целых поясов, .5 для получасовых).
    ...
    if data and "location" in data:
        loc = data["location"]
        tz_id = loc.get("tz_id")
        # Точный путь: localtime_epoch (секунды) минус реальное UTC (секунды).
        epoch_local = loc.get("localtime_epoch")
        if epoch_local:
            try:
                utc_now_epoch = _now_utc().timestamp()
                offset_hours = (float(epoch_local) - utc_now_epoch) / 3600.0
                # Округляем до ближайшей ПОЛОВИНЫ часа (поддержка UTC+5:30 и т. п.),
                # ограничиваем разумным диапазоном.
                tz_offset = round(offset_hours * 2) / 2.0
                if -12 <= tz_offset <= 14:
                    # Целые значения возвращаем как int (чтобы не ломать код,
                    # который делает int(tz)).
                    return int(tz_offset) if tz_offset == int(tz_offset) else tz_offset
            except Exception as e:
                logger.warning(
                    f"detect_timezone_for_city: WeatherAPI epoch fail: {e}"
                )
        else:
            # Fallback: строковый localtime (старое поведение, но с секундами UTC).
            ...
            if local_str:
                try:
                    ...
                    tz_offset = round(diff_hours * 2) / 2.0
                    if -12 <= tz_offset <= 14:
                        return int(tz_offset) if tz_offset == int(tz_offset) else tz_offset
                except Exception as e:
                    logger.warning(
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    if resp.status == 200:
                        ...
                        tz_offset = round(((raw + dst) / 3600.0) * 2) / 2.0
                        if -12 <= tz_offset <= 14:
                            return int(tz_offset) if tz_offset == int(tz_offset) else tz_offset
        except Exception as e:
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            # === НОВОЕ: 🛍 Магазин — создание товара (dev-FSM) ===
            DEV_PRODUCT_NAME: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, dev_product_name_handler),
                CallbackQueryHandler(handle_callback),
            ],
            DEV_PRODUCT_DESC: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, dev_product_desc_handler),
                CallbackQueryHandler(handle_callback),
            ],
            DEV_PRODUCT_PRICE_XTR: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, dev_product_price_xtr_handler),
                CallbackQueryHandler(handle_callback),
            ],
            DEV_PRODUCT_PRICE_VIRT: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, dev_product_price_virt_handler),
                CallbackQueryHandler(handle_callback),
            ],
            DEV_PRODUCT_GRANT: [
                CallbackQueryHandler(handle_callback),
            ],
            DEV_PRODUCT_GRANT_VALUE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, dev_product_grant_value_handler),
                CallbackQueryHandler(handle_callback),
            ],
            DEV_PRODUCT_PERIOD: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, dev_product_period_handler),
                CallbackQueryHandler(handle_callback),
            ],
