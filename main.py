...
STORAGE_CONFIG_DEFAULTS = {
    ...
    "channel_db_enabled": False,   # канал — первичное хранилище данных (сервер — кэш)
...
def get_main_menu_keyboard(user):
    ...
    standard_buttons = [
        ...
        # ВОЛНА 8 (критичный багфикс): «🔐 Сейф» была в ALL_MAIN_MENU_BUTTONS
        # и QUICK_COMMANDS, но НЕ была добавлена в standard_buttons этой
        # клавиатуры — из-за этого кнопки физически НЕ БЫЛО В МЕНЮ (пользователь
        # прав: «говорят про какую-то кнопку сейф, которой нет»). Маршрут в
        # handle_main_menu существует, поэтому достаточно добавить кнопку сюда.
        "🔐 Сейф",
        "⭐ Звезды", "🤖 DEVORKS+ai",
        "🪄 Автоматизация",
        "🌦 Погода",
...
async def _cdb_restore_on_boot(application):
    """ВОЛНА 8: восстановление базы ИЗ КАНАЛА при старте.

    Честная семантика «канал — первичная база»: если режим включён и в
    db-канале есть ЗАКРЕПЛЁННЫЙ наш снапшот — он и есть актуальные данные.
    Локальные файлы (кэш) перезаписываются снапшотом. Если канала/пина нет
    или канал недоступен — работаем на локальном кэше, как раньше, и при
    первом же изменении зальём его в канал.

    Вызывается из _post_init ПОСЛЕ запуска тикеров (в фоне, чтобы не мешать)."""
    try:
        cfg = load_storage_config()
        if not cfg.get("channel_db_enabled"):
            return
        db_ids = get_db_channel_ids()
        if not db_ids:
            logger.info("cdb boot: режим включён, но db-каналов нет — скип.")
            return
        bot = application.bot
        for ch in db_ids:
            try:
                ...
                logger.warning(f"cdb boot: get_chat({ch}) не удался: {e}")
                continue
            ...
            if doc is None or not fname.startswith(CDB_SNAPSHOT_PREFIX):
                logger.info(f"cdb boot: в канале {ch} нет закреплённого снапшота — скип.")
                continue
            try:
                ...
                logger.error(f"cdb boot: не удалось скачать снапшот из {ch}: {e}")
                continue
            # Верификация: это должен быть НАШ zip с _meta.json.
            try:
                zf = zipfile.ZipFile(io.BytesIO(payload))
                ...
                logger.error(f"cdb boot: снапшот в {ch} битый ({e}) — скип.")
                continue
            restored, problems = _storage_restore_apply(payload)
            if restored:
                # Восстановление само пометило файлы «грязными»; снапшот в
                # канале УЖЕ актуален (мы его оттуда взяли) — гасим «грязь»,
                # чтобы тикер не залил то же самое обратно.
                _CDB_DIRTY.clear()
                logger.info(
                    f"cdb boot: база восстановлена ИЗ КАНАЛА {ch} "
                    f"({fname}, {len(payload)} байт, файлов {len(restored)}); "
                    f"проблемы: {problems or 'нет'}"
                )
                return  # одного канала достаточно
            logger.warning(f"cdb boot: из {ch} ничего не восстановлено: {problems}")
    except Exception as e:
...
def _zip_bytes_for(name: str, data: bytes) -> bytes:
    """ВОЛНА 8: бот САМ заворачивает файл в ZIP (по кнопке пользователя).

    Архивация всегда БЕЗ потерь (это важно: пользователь просил «без потери
    качества»). Сжимаем DEFLATE'ом; для уже сжатых форматов это почти не
    даст выигрыша, но и не испортит файл — зато текст/документы жмутся в разы."""
    buf = io.BytesIO()
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
...
def _cloud_menu_text(user):
    if not get_cloud_channel_ids():
        return (
            ...
            "файлов без ограничения по количеству (до 49 МБ на файл — лимит Telegram "
            "для ботов; файлы больше 49 МБ можно завернуть в ZIP или разбить на части)."
        )
    files = getattr(user, "cloud_files", []) or []
    limit = get_price('cloud_max_files', 50)
    total = sum(int(f.get("size", 0) or 0) for f in files if isinstance(f, dict))
    return (
        "☁️ Личное облако\n\n"
        f"📁 Файлов: {len(files)} из {limit} • Размер: {_fmt_bytes(total)}\n\n"
        "Ваши файлы хранятся в приватном Telegram-канале — бесплатно и без "
        "ограничения по количеству. Доступ есть только у бота (он админ канала) "
        "и у вас через этого бота.\n\n"
        "Выберите действие:"
    )
...
def get_cloud_menu_keyboard():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("📤 Загрузить файл", callback_data="cloud_upload")],
        [InlineKeyboardButton("📁 Мои файлы", callback_data="cloud_files")],
        [InlineKeyboardButton("❓ Как это работает", callback_data="cloud_help")],
        [InlineKeyboardButton("⬅️ Назад", callback_data="back_to_main")],
    ])


def get_cloud_files_keyboard(user):
    """ВОЛНА 8: по файлу — ряд [📥 имя], под ним [📦 ZIP] [✏️ Имя] [🗑].
    📦 — бот сам заворачивает файл в ZIP (по просьбе пользователя),
    ...
    if not kb:
        kb.append([InlineKeyboardButton("📤 Загрузить файл", callback_data="cloud_upload")])
...
def _cloud_files_text(user):
    files = [f for f in (getattr(user, "cloud_files", []) or []) if isinstance(f, dict)]
    if not files:
        return "📁 В облаке пока нет файлов. Нажмите «📤 Загрузить файл» и отправьте документ, фото, видео или аудио."
    limit = get_price('cloud_max_files', 50)
    total = sum(int(f.get("size", 0) or 0) for f in files)
    shown = files[:30]
    lines = [f"📁 Ваши файлы ({len(files)}/{limit} • {_fmt_bytes(total)}):", ""]
    for rec in shown:
        lines.append(f"• {rec.get('name', '?')} — {_fmt_bytes(rec.get('size', 0))} ({rec.get('ts', '')})")
    if len(files) > len(shown):
        lines.append(f"…и ещё {len(files) - len(shown)} шт.")
    lines.append("")
    lines.append("Нажмите 📥 у файла — бот пришлёт его в чат. 🗑 — удалить.")
    return "\n".join(lines)
...
async def cloud_menu_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data.pop('cloud_file_mode', None)
    await update.message.reply_text(_cloud_menu_text(user), reply_markup=get_cloud_menu_keyboard())
    return MAIN_MENU
...
async def cloud_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data.pop('cloud_file_mode', None)
    try:
        await query.edit_message_text(_cloud_menu_text(user), reply_markup=get_cloud_menu_keyboard())
    except Exception:
        await query.message.reply_text(_cloud_menu_text(user), reply_markup=get_cloud_menu_keyboard())
    return MAIN_MENU
...
async def cloud_help_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "2. Вы отправляете боту файл (документ/фото/видео/аудио) — бот кладёт его "
        "в канал (файлы распределяются по кругу между каналами). Telegram хранит "
        "файлы бесплатно и бессрочно.\n"
        "3. 📎 АЛЬБОМЫ И ПАЧКИ: можно отправить сразу НЕСКОЛЬКО файлов или альбом "
        "(группу фото/видео) — я сохраню всё пачкой и покажу один итог.\n"
        "4. ✏️ ПОДПИСИ: подпишите файл при отправке (caption) — подпись станет "
        "именем в облаке. Переименовать позже: «📁 Мои файлы» → ✏️.\n"
        "5. 📦 ZIP ОДНОЙ КНОПКОЙ: в «Мои файлы» есть кнопка «📦 ZIP» — я сам "
        "скачаю файл, заверну в архив (БЕЗ потери качества) и заменю им оригинал. "
        "Текст/документы жмутся в 3–10 раз; фото/видео — почти нет (они уже "
        "сжаты форматом).\n"
        "6. 📁 Список ваших файлов — «📁 Мои файлы». По кнопке 📥 бот мгновенно "
        "присылает файл обратно в любой чат с вами.\n"
        "7. 🔐 Сейф — то же самое, но файлы ещё и ШИФРУЮТСЯ вашим паролем: без "
        "пароля их не откроет никто, даже разработчик.\n\n"
        "Честные ограничения:\n"
        "• обычная загрузка — до 49 МБ на файл (лимит Telegram Bot API). Файл "
        "больше — кнопка «📤 Отправить в канал самому»: пришлёте файл прямо в "
        "канал (до 2 ГБ!), я запишу его в ваше облако;\n"
        "• «📦 ZIP» работает с файлами до 20 МБ — больше бот физически не может "
        "СКАЧАТЬ (лимит ботов), это не обходится;\n"
    ...
    try:
        await query.edit_message_text(text, reply_markup=get_cloud_menu_keyboard())
    except Exception:
        await query.message.reply_text(text, reply_markup=get_cloud_menu_keyboard())
    return MAIN_MENU
    ...
    if not get_cloud_channel_ids():
        try:
            await query.edit_message_text(
                ...
                "🛠️ Панель разработчика → ☁️ Хранилище → 🔗 Каналы.",
                reply_markup=get_cloud_menu_keyboard(),
            )
        except Exception:
            pass
        return MAIN_MENU
    # Флаг для STT-middleware: в режиме загрузки аудио/голосовые НЕ
    # транскрибируются — пользователь сохраняет сам файл.
    context.user_data['cloud_file_mode'] = True
    files = getattr(user, 'cloud_files', []) or []
    limit = get_price('cloud_max_files', 50)
    try:
        await query.edit_message_text(
            f"📤 Режим загрузки ({len(files)}/{limit})\n\n"
            "Отправьте документ, фото, видео или аудио — я сохраню его в облако.\n"
            "📎 Можно сразу НЕСКОЛЬКО файлов или АЛЬБОМ — сохраню пачкой.\n"
            "✏️ Подпишите файл (caption) — подпись станет именем в облаке.\n"
            "Голосовые и аудио тоже сохраняются КАК ФАЙЛЫ, а не расшифровываются."
        )
    except Exception:
        pass
...
def _vault_menu_text(user):
    ...
    return (
        ...
        "Ваши файлы шифруются ВАШИМ паролем (AES-256, ключ выводится из "
        "пароля 600 000 раундами). В канале и базе лежит только шум — открыть "
        "не сможет никто: ни разработчик, ни владелец сервера, ни Telegram.\n\n"
        "⚠️ Пароль нигде не хранится. Забыли пароль — файл не откроет "
        "УЖЕ НИКТО, навсегда. Это честная цена настоящей приватности.\n\n"
        "Выберите действие:"
    )
...
def _vault_help_text():
    return (
        "❓ Как работает Сейф\n\n"
        "1. Вы присылаете файл (документ/фото/видео/аудио, до 20 МБ) или "
        "текст. Бот просит придумать ПАРОЛЬ.\n"
        "2. Ключ шифрования выводится из пароля (PBKDF2, 600 000 раундов) и "
        "содержимое шифруется AES-256-GCM. В Telegram-канал уходит ТОЛЬКО "
        "шифр — неразличимый от случайного шума.\n"
        "3. Чтобы достать файл, вы вводите пароль: бот скачивает шифр, "
        "расшифровывает в памяти и присылает файл. На диск ничего не пишется.\n\n"
        "Почему никто не откроет:\n"
        "• пароль не хранится нигде (проверка — по «отпечатку» ключа);\n"
        ...
        "• ZIP не нужен для мелких файлов, но если файл чуть больше 20 МБ — "
        "сожмите его в ZIP (текст сожмётся в разы) и положите в Сейф архив;\n"
        "• забыли пароль — данные потеряны навсегда, запасного ключа НЕТ."
    )
...
async def vault_put_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await query.edit_message_text(
            ...
            f"{_fmt_bytes(VAULT_MAX_FILE_BYTES)}) — или просто НАПИШИТЕ текст "
            "сообщением (до 4000 символов): он сохранится как зашифрованная "
            "заметка.\n\n"
            "Дальше бот попросит придумать пароль. Пароль НЕ сохраняется: "
            "потеряете — файл не откроет уже никто."
        )
    except Exception:
        pass
    await query.message.reply_text(
...
async def vault_put_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Принимает файл/текст для шифрования. Файл скачивается в ПАМЯТЬ
    (Telegram разрешает ботам качать только ≤20 МБ — этим же лимитом и
    ограничены файлы Сейфа), шифруется на следующем шаге (пароль)."""
    msg = update.message
    user_id = str(update.effective_user.id)
    user = get_user(user_id)
    if not user:
    ...
    if att is None:
        # Текст → зашифрованная заметка (txt).
        text = (msg.text or "").strip()
        if not text:
            await msg.reply_text(
                "🔐 Пришлите ФАЙЛ (документ/фото/видео/аудио) или НАПИШИТЕ "
                "текст — он станет зашифрованной заметкой. Чтобы выйти: "
                "«⬅️ Назад в меню»."
            )
            return VAULT_PUT_WAIT
        if len(text) > VAULT_MAX_TEXT_CHARS:
            ...
            return VAULT_PUT_WAIT
        context.user_data['vault_pending'] = {
        ...
        if fsize > VAULT_MAX_FILE_BYTES:
            ...
            await msg.reply_text(
                ...
                "вообще не даёт ботам скачивать такие файлы, значит шифровать "
                "их честно невозможно.\n\n"
                "Что делать:\n"
                "• сожмите файл в ZIP (текст сожмётся в разы; видео/фото — "
                "почти нет, они уже сжаты);\n"
                "• или положите его в обычное ☁️ Облако без шифрования "
                "(туда можно до 49 МБ, а больше — частями)."
            )
            return VAULT_PUT_WAIT
        if kind == "photo":
        ...
        payload = b""  # освобождаем локальную ссылку; живёт только в user_data

    await msg.reply_text(
        "🔑 Придумайте ПАРОЛЬ для этого файла (минимум 6 символов).\n\n"
        "⚠️ Пароль будет удалён из чата, но НЕ будет сохранён нигде. Если "
        "забудете его — файл не откроет уже НИКТО, включая меня.\n\n"
        "Отправьте пароль одним сообщением (отменить: /cancel).",
...
async def vault_put_password(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Шифрует отложенный файл введённым паролем и грузит шифр в канал."""
    msg = update.message
    user_id = str(update.effective_user.id)
    user = get_user(user_id)
    pending = context.user_data.get('vault_pending')
    if not user or not pending:
        context.user_data.pop('vault_put_mode', None)
        context.user_data.pop('vault_pending', None)
        await msg.reply_text(
    ...
    if _pw_low in ("отмена", "cancel", "/cancel", "❌ отмена") or "отмена" in _pw_low or "cancel" in _pw_low:
        ...
        context.user_data.pop('vault_pending', None)
        await msg.reply_text(
            "Отменено — файл НЕ зашифрован и НЕ загружен.",
            reply_markup=get_main_menu_keyboard(user),
        )
        return MAIN_MENU
    if len(password) < VAULT_MIN_PASSWORD:
        ...
        return VAULT_PUT_PASSWORD
    # Шифруем (в памяти) и сразу стираем исходник из user_data.
    try:
        container = await asyncio.to_thread(
            _vault_pack, password, pending["payload"], {
                "n": pending["name"], "k": pending["kind"],
                "m": pending["mime"], "t": datetime.now().strftime("%Y-%m-%d %H:%M"),
            },
        )
    except Exception as e:
        logger.error(f"vault put: шифрование не удалось: {e}")
        # Исходник уже стёрт (finally) — честно отправляем на повтор.
        context.user_data.pop('vault_pending', None)
        await msg.reply_text(
            "❌ Сбой шифрования. Пришлите файл ещё раз: 🔐 Сейф → 📥 Положить.",
            reply_markup=get_vault_menu_keyboard(),
        )
        return MAIN_MENU
    finally:
        # В любом случае исходные байты больше не нужны.
        pending["payload"] = b""
    size_orig = int(pending.get("size", 0) or 0)
    ...
    up = await _storage_upload_document(
        context, container,
        filename=f"vault_{datetime.now().strftime('%Y%m%d_%H%M%S')}.bin",
        caption="🔐 Сейф: зашифрованный файл (открыть без пароля невозможно).",
    )
    container = b""
    if up is None:
        await msg.reply_text(
            "❌ Не удалось загрузить шифр в канал. Файл НЕ сохранён — попробуйте "
            "ещё раз: 🔐 Сейф → 📥 Положить.",
            reply_markup=get_vault_menu_keyboard(),
        )
        return MAIN_MENU
    files = [f for f in (getattr(user, "vault_files", []) or []) if isinstance(f, dict)]
    rec = {
        ...
        "kind": pending.get("kind", "document"),
    ...
    context.user_data.pop('vault_pending', None)
    await msg.reply_text(
        "✅ ФАЙЛ ЗАШИФРОВАН И ЗАГРУЖЕН.\n\n"
        "• В канале теперь лежит только шифр — открыть его нельзя без пароля.\n"
        "• Пароль стёрт из чата и нигде не хранится.\n"
        "• Даже имя файла спрятано в шифре.\n\n"
        "⚠️ Запомните пароль прямо сейчас: забыли — файл потерян навсегда.",
        reply_markup=get_main_menu_keyboard(user),
    )
    return MAIN_MENU
...
async def vault_get_password(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    Также обслуживает ШАГ 1 смены пароля (vault_chp_stage == "old")."""
    msg = update.message
    user = get_user(str(update.effective_user.id))
    ...
    if not user or (not vid and chp_stage != "old"):
        await msg.reply_text(
...
def _storage_dev_status_text(cfg):
    ...
    lines.append(
        ...
        "угодно. Бот не читает историю канала: реестр — в storage_config.json."
    )
    return "\n".join(lines)
...
async def dev_storage_channel_kind(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        "между ☁️-каналами, бэкап базы зеркалится во все 🗄️-каналы.\n\n"
...
async def _post_init(application):
    ...
    # === ШАГ 3 (ВОЛНА 8): восстановление базы ИЗ КАНАЛА (канал = база). ===
    # В фоне, чтобы не задерживать старт: если режим включён и в db-канале
    # есть закреплённый снапшот — он становится текущими данными.
    try:
