...
def _automation_system_prompt(context_text, is_admin):
    ...
    return (
        ...
        '17) {"action":"clarify","question":"<один конкретный вопрос>"} — если данных не хватает или запрос неоднозначен.\n'
        '18) {"action":"none","answer":"<ответ на общий вопрос>"} — запрос не про школьные действия: просто ответь по-русски.\n\n'
        "ПРАВИЛА:\n"
...
def _automation_help_text():
    return (
        ...
        "• «Какое ДЗ на понедельник?»\n"
        "• «Покажи учителей» / «Расписание на пятницу»\n\n"
        "Если данных не хватает — я задам уточняющий вопрос.\n"
        "Изменять класс могут только админы класса.\n\n"
        "Выйти: «отмена»."
    )
...
def get_shop_keyboard(user):
    ...
    for product in sorted(products, key=lambda p: str(p.get("created_at", ""))):
        ...
        keyboard.append([InlineKeyboardButton(
            f"{product.get('name', 'Товар')} — {_product_label_price(product)}",
...
async def dev_product_desc_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Шаг 3/6 — цена в Telegram Stars (XTR)\n\nВведите ЦЕЛОЕ ЧИСЛО (например 50).\n"
...
async def dev_product_price_virt_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Шаг 5/6 — какую услугу зачислять после оплаты?",
        reply_markup=kb,
    )
...
async def dev_product_grant_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if grant_type == "stars":
        await query.edit_message_text(
            "Шаг 6/6 — сколько ВИРТУАЛЬНЫХ звёзд зачислять после оплаты? Введите целое число:",
            reply_markup=get_cancel_keyboard(),
        )
        return DEV_PRODUCT_GRANT_VALUE
    # custom
    await query.edit_message_text(
        "Шаг 6/6 — назовите услугу (например «консультация», «разбор ДЗ»):",
        reply_markup=get_cancel_keyboard(),
    )
    return DEV_PRODUCT_GRANT_VALUE
...
async def precheckout_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if payload.startswith("buy_stars_"):
        ...
        # Пропускаем только если товар существует и активен.
        _pid = payload[len(PRODUCT_PAY_PAYLOAD_PREFIX):]
        _product = get_product(_pid)
        if _product and _product.get("active", True):
            await query.answer(ok=True)
        else:
