...
def _storage_add_channel(chat_id, title, kind):
    ...
    for ch in (cfg.get("channels") or []):
        if isinstance(ch, dict) and ch.get("id") == int(chat_id):
            ...
            return cfg, False
    (cfg.setdefault("channels", []) or []).append({
        "id": int(chat_id),
...
async def _storage_do_backup(context):
    ...
    for const in dict.fromkeys(STORAGE_BACKUP_FILES):
        ...
        try:
            ...
            logger.error(f"storage backup: {base} не сериализуется: {e}")
            continue
    if not files:
    ...
    caption = (
        ...
        f"файлов {len(files)} • {_fmt_bytes(len(payload))}"
    )
...
def _big_file_advice_text(fsize: int = 0) -> str:
    ...
    return (
        ...
        "такие файлы в канал. Но есть пути:\n\n"
...
def get_cloud_files_keyboard(user):
    ...
    for rec in files[:30]:
        ...
        label = f"{name[:36]} ({size})"
        kb.append([
            InlineKeyboardButton(f"📥 {label}", callback_data=f"cloud_get_{rec.get('id')}"),
...
async def cloud_help_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "3. Список ваших файлов — «📁 Мои файлы». По кнопке 📥 бот мгновенно "
        "присылает файл обратно в любой чат с вами.\n"
        "4. 🔐 Сейф — то же самое, но файлы ещё и ШИФРУЮТСЯ вашим паролем: без "
        "пароля их не откроет никто, даже разработчик.\n\n"
        "Честные ограничения:\n"
        "• до 49 МБ на один файл (лимит Telegram Bot API). Файл больше — заверните "
        "в ZIP (без потери качества; текст сожмётся в разы, фото/видео — почти "
        "нет) или разбейте архив на томы по 45 МБ и пришлите части — я сохраню "
        "каждую;\n"
        "• лимит файлов на человека задаёт разработчик (по умолчанию 50);\n"
        "• бот не может читать историю канала — только файлы, которые загрузил через себя."
    )
    try:
...
async def cloud_upload_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Принимает файл в режиме загрузки и кладёт его в приватный канал.

    Оптимизация: файл НЕ скачивается — бот пересылает его в канал по file_id
    (Telegram разрешает ботам переотправку), после чего в БД сохраняются
    только указатели (message_id/file_id нового сообщения в канале)."""
    msg = update.message
    user_id = str(update.effective_user.id)
    user = get_user(user_id)
    if not user:
    ...
    if not cloud_ids:
        ...
        return MAIN_MENU
    # Канал для ЭТОГО файла: берём из указателя круговой загрузки, чтобы файлы
    # равномерно распределялись по всем cloud-каналам.
    try:
    ...
    if fsize > STORAGE_MAX_FILE_BYTES:
        # ВОЛНА 7: не просто отказ, а честные варианты решения — ZIP без
        # потери качества, разбивка на томы, подсказка про Сейф.
        await msg.reply_text(_big_file_advice_text(fsize))
        return CLOUD_UPLOAD_WAIT
    ...
    if len(files) >= limit:
        await msg.reply_text(
            f"🚫 Достигнут лимит облака ({len(files)}/{limit} файлов). Удалите "
            "что-нибудь через «☁️ Облако → 📁 Мои файлы» (лимит меняется в панели цен)."
        )
        return CLOUD_UPLOAD_WAIT
    ...
    if kind == "photo":
        ...
        name = getattr(att, "file_name", None) or f"Документ_{ts_fn}"
    elif kind == "video":
    ...
    # Пересылаем файл в канал по file_id (без скачивания).
    try:
        if kind == "photo":
            sent = await context.bot.send_photo(chat_id=channel_id, photo=att.file_id, caption=name[:1024])
            new_file_id = getattr(getattr(sent, "photo", [None])[-1], "file_id", None) if getattr(sent, "photo", None) else None
        elif kind == "video":
            sent = await context.bot.send_video(chat_id=channel_id, video=att.file_id, caption=name[:1024])
            new_file_id = getattr(getattr(sent, "video", None), "file_id", None)
        elif kind == "audio":
            sent = await context.bot.send_audio(chat_id=channel_id, audio=att.file_id, caption=name[:1024])
            new_file_id = getattr(getattr(sent, "audio", None), "file_id", None)
        elif kind == "voice":
            sent = await context.bot.send_voice(chat_id=channel_id, voice=att.file_id, caption=name[:1024])
            new_file_id = getattr(getattr(sent, "voice", None), "file_id", None)
        else:
            sent = await context.bot.send_document(chat_id=channel_id, document=att.file_id, caption=name[:1024])
            new_file_id = getattr(getattr(sent, "document", None), "file_id", None)
    except TGBadRequest as e:
        await msg.reply_text(f"❌ Telegram отклонил файл: {e.message}")
        return CLOUD_UPLOAD_WAIT
    except Exception as e:
        logger.error(f"cloud upload: {e}")
        await msg.reply_text("❌ Не удалось загрузить файл в хранилище. Попробуйте ещё раз.")
        return CLOUD_UPLOAD_WAIT
    ...
    rec = {
        ...
        "file_id": new_file_id,
        "size": fsize or int(getattr(att, "file_size", 0) or 0),
        "mime": str(getattr(att, "mime_type", "") or ""),
        "ts": ts,
        # ВОЛНА 7: в каком именно канале лежит файл (каналов теперь несколько).
        "channel_id": channel_id,
    }
    files.append(rec)
    ...
    if len(cloud_ids) > 1:
        try:
            cfg_rr = load_storage_config()
            cfg_rr["cloud_rr"] = (_rr + 1) % len(cloud_ids)
            save_storage_config(cfg_rr)
        except Exception:
            pass
    _zip_tip = ""
    if (kind == "document" and fsize >= 512 * 1024
            and _is_compressible_name(name, str(getattr(att, "mime_type", "") or ""))):
        _zip_tip = ("\n💡 Совет: это текстовый файл — в ZIP он сожмётся в 3–10 раз "
                    "без потери качества. Пришлите его в ZIP, чтобы беречь место.")
    await msg.reply_text(
        f"✅ Сохранено в облако: «{rec['name']}» ({_fmt_bytes(rec['size'])}).\n"
        f"Файлов: {len(files)}/{limit}. Можно отправить ещё или «⬅️ Назад в меню»."
        f"{_zip_tip}"
    )
    return CLOUD_UPLOAD_WAIT
...
def _automation_class_context_text(user, class_obj):
    ...
    # Хранилище/облако — контекст для действий show_storage/backup_now/vault_status.
    try:
