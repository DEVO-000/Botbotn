...
from telegram.ext import Application, CommandHandler, MessageHandler, filters, ContextTypes, CallbackQueryHandler, ConversationHandler, PreCheckoutQueryHandler, ApplicationHandlerStop
...
# Для автоматизации (⚡ Автоматизация):
#   DEEPSEEK_API_KEY  — ключ DeepSeek API (https://platform.deepseek.com)
# Опционально:
#   GROQ_MODEL     — модель Groq (по умолчанию llama-3.3-70b-versatile)
#   DEEPSEEK_MODEL — модель DeepSeek (по умолчанию deepseek-chat)
#   AUTOMATION_PROVIDER — "deepseek" (по умолчанию) или "groq"
...
# === АВТОМАТИЗАЦИЯ (DeepSeek API) ===
# Ключ DeepSeek (https://platform.deepseek.com) для модуля «⚡ Автоматизация»:
# бот превращает фразы вида «русский на пятницу этой недели страница 45»
# в готовое ДЗ, «замени учителя по биологии» — в замену учителя и т. д.
DEEPSEEK_API_KEY = _env("DEEPSEEK_API_KEY")
# Модель DeepSeek: deepseek-chat (обычный) или deepseek-reasoner (с рассуждениями).
DEEPSEEK_MODEL = _env("DEEPSEEK_MODEL", "deepseek-chat")
# Базовый URL API DeepSeek (OpenAI-совместимый эндпоинт).
DEEPSEEK_BASE_URL = _env("DEEPSEEK_BASE_URL", "https://api.deepseek.com")
# Провайдер автоматики: "deepseek" (по умолчанию) или "groq" (запасной).
# Если выбран DeepSeek, но ключ не задан, а Groq-ключ есть — автоматика
# временно работает через Groq (в логах будет предупреждение), чтобы функция
# не «умирала» молча до настройки ключа.
AUTOMATION_PROVIDER = _env("AUTOMATION_PROVIDER", "deepseek").lower()
if AUTOMATION_PROVIDER not in ("deepseek", "groq"):
    AUTOMATION_PROVIDER = "deepseek"
...
if not DEEPSEEK_API_KEY and AUTOMATION_PROVIDER == "deepseek" and GROQ_API_KEY:
    logging.getLogger(__name__).warning(
        "DEEPSEEK_API_KEY не задан — автоматизация временно работает через Groq. "
        "Задайте DEEPSEEK_API_KEY (https://platform.deepseek.com), чтобы "
...
# === НОВЫЕ состояния (автоматизация, платежи, платные кнопки-товары) ===
# Ввод времени окончания звонка (второй шаг после EDIT_BELL_TIME — чинит
# «бесконечный цикл» редактирования звонков: раньше конец никогда не вводился).
EDIT_BELL_END = 111
# Ввод произвольной суммы покупки звёзд (Telegram Stars, динамический чек).
BUY_STARS_CUSTOM = 112
# Ввод цены кастомной кнопки-товара класса (0 — бесплатная).
CUSTOM_BUTTON_PRICE = 113
# Ввод цены глобальной кнопки-товара (0 — бесплатная).
DEV_GLOBAL_BUTTON_PRICE = 114
...
def load_prices():
    default_prices = {
        ...
        'anon_keep_month': 100,
        # ПУНКТ 2 (синхронизация цен): стоимость одной генерации DEVO+ai
        # во внутренних звёздах. 0 — генерации бесплатны (как раньше);
        # разработчик задаёт цену в панели «Быстрое изменение цен», и она
        # сразу применяется во всём боте.
        'ai_generation': 0,
    }
...
class CustomButton:
    def __init__(self, button_id, class_code, name, content, creator_id, button_type="text", price=0):
        self.button_id = button_id
        self.class_code = class_code
        self.name = name
        self.content = content
        ...
        self.created_date = datetime.now().strftime("%Y-%m-%d %H:%M")
        self.is_active = True
        # ПУНКТ 1 (платные кнопки-товары): цена в звёздах. 0 — бесплатная
        # кнопка (как раньше). Положительная цена превращает кнопку в товар:
        # при клике бот выставляет счёт и мгновенно выдаёт содержимое
        # после оплаты.
        try:
...
class GlobalButton:
    def __init__(self, button_id, name, content, button_type="text", price=0):
        self.button_id = button_id
        self.name = name
        self.content = content
        self.button_type = button_type
        self.created_date = datetime.now().strftime("%Y-%m-%d %H:%M")
        self.is_active = True
        # ПУНКТ 1 (платные кнопки-товары): цена в звёздах (0 — бесплатно).
        try:
            self.price = max(0, int(price))
        except (TypeError, ValueError):
            self.price = 0
...
def load_stars_stats():
    """Загрузка агрегированной статистики звёзд.

    ПУНКТ 3 (обновление статистики Stars): подробная история переводов
    (список transactions) больше НЕ хранится — при загрузке она полностью
    удаляется из файла (одноразовая миграция), остаются только агрегаты:
    общее число пользователей, суммарно потраченные звёзды и топ трат.
    """
    ...
    if 'transactions' in stats:
        # Полная очистка подробной истории переводов.
        try:
            stats.pop('transactions', None)
            changed = True
        except Exception:
            pass
    stats.setdefault('total_stars_spent', 0)
    stats.setdefault('top_donors', [])
...
def save_stars_stats(stats):
    # Страховка: если кто-то передал словарь со старым ключом transactions —
    # вычищаем его перед записью, чтобы история не накапливалась снова.
    if isinstance(stats, dict) and 'transactions' in stats:
        stats.pop('transactions', None)
...
def _default_instructions_text():
    return (
        ...
        '**📋 ОСНОВНЫЕ ФУНКЦИИ:**\n'
        '1. **Регистрация** — укажите дату рождения и город (часовой пояс определится автоматически).\n'
        ...
        '8. **Погода и праздники** — ежедневный прогноз по вашему городу в заданное время (⚡ настройки погоды) и напоминания о праздниках. Погоду можно запросить и кнопкой.\n'
        ...
        '12. **⚡ Автоматизация** — управляйте ботом обычными фразами: «русский на пятницу этой недели страница 45» → бот сам создаст ДЗ на пятницу; «замени учителя по биологии, географии и обществознанию на Иванову» → бот заменит учителей. Работает через DeepSeek API. Изменения показываются на подтверждение перед выполнением.\n\n'
        '**⌨️ БЫСТРЫЕ КОМАНДЫ (работают в любом месте бота):**\n'
        '/dz или /дз — домашнее задание (можно с датой: /dz завтра)\n'
        '/today /сегодня · /tomorrow /завтра — расписание на день\n'
        '/schedule /расписание — расписание на неделю\n'
        '/teachers /учителя — список учителей\n'
        '/bells /звонки — звонки\n'
        '/weather /погода — погода сейчас\n'
        ...
        'Сумму покупки звёзд можно выбрать любую («📐 Своя сумма»).\n'
        '• 🛍 **Платные кнопки-товары** — админ класса и разработчик могут создавать кнопки с ценой в звёздах: '
        'ученики оплачивают товар, и содержимое кнопки доставляется мгновенно.\n'
        '• 💬 **Чат поддержки** — кнопка «💬 Чат поддержки» в главном меню. '
        'Напишите разработчику напрямую и получите ответ в этом же чате.\n'
        '• 🌦 **Погода** — ежедневно в ваше время (по часовому поясу вашего города). '
        'Отключается в ⚙️ Настройки → 🌦 Настройки погоды.\n'
        ...
        '• Место под анонимки (месяц) — {anon_keep_month} ⭐\n'
        '• Генерация DEVO+ai — {ai_generation} ⭐ (0 — бесплатно)\n\n'
...
def get_instructions():
    ...
    try:
        ...
        text = text.format(
            ...
            # ПУНКТ 2 (синхронизация цен): новые ключи тоже подставляются.
            # Если в кастомной инструкции этих плейсхолдеров нет — format
            # просто их игнорирует (лишние kwargs не вызывают ошибку).
            anon_keep_month=prices.get('anon_keep_month', 100),
            ai_generation=prices.get('ai_generation', 0),
        )
...
def _update_stars_aggregates():
    """Пересчитывает агрегаты статистики звёзд из users.json.

    ПУНКТ 3 (обновление статистики Stars): единственный источник правды о
    тратах — поле total_stars_spent у пользователей. Подробная история
    переводов не хранится и не пересчитывается по транзакциям.
    """
    try:
        users = load_users()
        total_spent = 0
        ...
        for u in users.values():
            try:
                spent = int(getattr(u, 'total_stars_spent', 0) or 0)
            except (TypeError, ValueError):
                spent = 0
            if spent > 0:
                spenders.append({
                    'user_id': str(getattr(u, 'user_id', '')),
                    'user_name': getattr(u, 'first_name', None) or 'Пользователь',
                    'total_spent': spent,
                })
            total_spent += spent
        spenders.sort(key=lambda x: x['total_spent'], reverse=True)
        stats = load_stars_stats()
        stats['total_stars_spent'] = total_spent
        stats['top_donors'] = spenders[:10]
        stats['total_users'] = len(users)
        save_stars_stats(stats)
        return stats
    except Exception as e:
        logger.error(f"_update_stars_aggregates: {e}")
        return load_stars_stats()


def add_stars_transaction(user_id, amount, description=""):
    """Изменение внутреннего баланса звёзд пользователя.

    amount > 0 — начисление (пополнение баланса),
    amount < 0 — списание с баланса (увеличивает total_stars_spent).

    ПУНКТ 3: подробные записи о переводах больше НЕ сохраняются — только
    агрегированная статистика (общее число пользователей, суммарные траты,
    топ трат), которая пересчитывается из users.json.
    """
    user = get_user(user_id)
    if not user:
        return False
    ...
    save_user(user)
    _update_stars_aggregates()
    return True


def record_stars_purchase(user_id, amount, description=""):
    """Учитывает покупку услуги за РЕАЛЬНЫЕ Telegram Stars (XTR).

    В отличие от add_stars_transaction, здесь не трогается внутренний баланс:
    пользователь оплатил услугу настоящими звёздами Telegram, поэтому только
    увеличиваем total_stars_spent и пересчитываем агрегаты. Так статистика
    честно отражает все оплаты — и внутренние, и через XTR.
    """
    user = get_user(user_id)
    if not user:
        return False
    ...
    if amount <= 0:
        return False
    user.total_stars_spent = int(getattr(user, 'total_stars_spent', 0) or 0) + amount
    save_user(user)
    _update_stars_aggregates()
    return True
...
def parse_tz_hours(tz_raw, default=3.0):
    """ПУНКТ «Погода»: парсит часовой пояс пользователя в число часов.

    Поддерживает int, float (например 5.5 для UTC+5:30) и строки вида
    «+3», «UTC+3», «UTC+5:30». При любой ошибке возвращает default.
    """
    if tz_raw is None:
        return default
    if isinstance(tz_raw, (int, float)):
        try:
            ...
            return default
    s = str(tz_raw).strip().upper().replace("UTC", "").replace(" ", "")
    if not s:
        return default
    # «+5:30» → «+5.5»
    if ":" in s:
...
def format_tz_offset(tz_raw):
    """Красиво форматирует смещение: «UTC+3», «UTC+5:30», «UTC-4»."""
    tz = parse_tz_hours(tz_raw)
    sign = "+" if tz >= 0 else "−"
    a = abs(tz)
    if a == int(a):
        return f"UTC{sign}{int(a)}"
    whole = int(a)
    minutes = int(round((a - whole) * 60))
    if minutes == 0:
        return f"UTC{sign}{whole}"
    return f"UTC{sign}{whole}:{minutes:02d}"


def get_local_time(user):
    utc_time = datetime.utcnow()
    tz = parse_tz_hours(getattr(user, 'timezone', 3) if user else 3)
...
def get_stars_prices_keyboard(user=None):
    ...
    ПУНКТ 1 (динамические чеки): добавлена кнопка «📐 Своя сумма» —
    бот создаёт чек оплаты на ЛЮБУЮ сумму, а не только на фиксированные
    пакеты 100/200/500/1000.
    """
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📐 Своя сумма", callback_data="buy_stars_custom")],
        [InlineKeyboardButton("❌ Отмена", callback_data="cancel_action")]
    ])
...
async def handle_ai_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ПУНКТ 2 (синхронизация цен): стоимость одной генерации берётся из
    # общей панели цен PRICES['ai_generation']. Значение 0 = генерации
    # бесплатны (прежнее поведение). Разработчик меняет цену в панели
    # «Быстрое изменение цен» — и новая цена мгновенно применяется во всём
    # боте, без перезапуска.
    try:
        ai_price = int(PRICES.get('ai_generation', 0) or 0)
    except (TypeError, ValueError):
        ai_price = 0
    if ai_price > 0:
        payer = get_user(user_id)
        if payer is None:
            payer = User(user_id)
            save_user(payer)
        if int(getattr(payer, 'stars_balance', 0) or 0) < ai_price:
            await update.message.reply_text(
                f"💎 Генерация DEVO+ai сейчас стоит {ai_price} ⭐.\n"
                f"На вашем балансе {payer.stars_balance} ⭐ — недостаточно.\n\n"
                f"Пополните баланс звёздами, чтобы продолжить:",
    ...
    # ПУНКТ 2 (синхронизация цен): списываем стоимость генерации только
    # после УСПЕШНОГО ответа модели. Если Groq не ответил — деньги не берём.
    if ai_price > 0:
        try:
            add_stars_transaction(user_id, -ai_price, "Генерация DEVO+ai")
        except Exception as e:
            logger.error(f"AI billing failed for {user_id}: {e}")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # /start всегда выключает режим автоматизации и AI, чтобы после
    # перезапуска регистрации флаги-оверлеи не перехватывали текст.
    try:
        context.user_data['auto_mode'] = False
        context.user_data['ai_mode'] = False
        context.user_data.pop('pending_auto_intent', None)
    except Exception:
        pass
...
async def restore_session(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = str(update.effective_user.id)
    # Восстановление сессии выходит из режимов-оверлеев.
    try:
        context.user_data['auto_mode'] = False
        context.user_data['ai_mode'] = False
        context.user_data.pop('pending_auto_intent', None)
...
async def cancel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # /cancel также выходит из режимов-оверлеев (автоматизация/AI).
    try:
        context.user_data['auto_mode'] = False
        context.user_data['ai_mode'] = False
        context.user_data.pop('pending_auto_intent', None)
...
async def handle_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for button in global_buttons:
        if button.name == message_text:
            # ПУНКТ 1 (платные кнопки-товары): глобальная кнопка с ценой —
            # сначала оплата (внутренние ⭐ или Telegram Stars), потом выдача.
            btn_price = int(getattr(button, 'price', 0) or 0)
            if btn_price > 0 and str(user_id) != str(DEVELOPER_ID):
                return await offer_product_purchase(update, context, user, button)
            if button.button_type == "url":
                await update.message.reply_text(
                    f"🔗 {button.name}",
    ...
    if class_obj and not is_user_class_blocked(user_id, class_obj.class_code):
        ...
        for custom_button in custom_buttons:
            ...
            if button_text == message_text:
                # ПУНКТ 1 (платные кнопки-товары): классная кнопка с ценой.
                # Создатель и админы класса получают содержимое бесплатно.
                btn_price = int(getattr(custom_button, 'price', 0) or 0)
                is_class_admin = str(user_id) in class_obj.admins or str(user_id) == class_obj.creator_id
                if btn_price > 0 and not is_class_admin and str(user_id) != str(DEVELOPER_ID):
                    return await offer_product_purchase(update, context, user, custom_button)
    ...
    if message_text == "🎓 Управление классами":
        ...
        # Вход в режим автоматизации (DeepSeek): фразы вида «русский на
        # пятницу этой недели страница 45» превращаются в готовое ДЗ.
        await enter_automation_mode(update, context, user)
        return MAIN_MENU
...
async def buy_stars_custom_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """ПУНКТ 1 (динамические чеки): ввод своей суммы покупки звёзд.

    Любая сумма от 1 до 2500 XTR — бот создаёт чек оплаты динамически,
    под конкретное число, без фиксированных пакетов.
    """
    query = update.callback_query
    await query.answer()
    try:
        await query.edit_message_text(
            "📐 Введите сумму звёзд для покупки (целое число от 1 до 2500):\n\n"
            "Бот создаст чек в Telegram Stars (XTR) на указанную сумму — "
            "1 внутренняя звезда = 1 XTR.",
            reply_markup=get_cancel_keyboard(),
        )
    except Exception:
        await context.bot.send_message(
            chat_id=query.from_user.id,
            text="📐 Введите сумму звёзд для покупки (целое число от 1 до 2500):",
            reply_markup=get_cancel_keyboard(),
        )
    return BUY_STARS_CUSTOM


@timeout(CONVERSATION_TIMEOUT)
async def buy_stars_custom_amount_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """ПУНКТ 1: создание динамического чека на введённую сумму."""
    user_id = str(update.effective_user.id)
    user = get_user(user_id)
    if not user:
        user = User(user_id)
    ...
    try:
        ...
        await update.message.reply_text("Введите целое число, например: 250")
        return BUY_STARS_CUSTOM

    if amount < 1 or amount > 2500:
        await update.message.reply_text(
            "Сумма должна быть от 1 до 2500 ⭐ (лимит Telegram на один чек).\n"
            "Попробуйте ещё раз:"
        )
        return BUY_STARS_CUSTOM
    ...
    try:
        ...
        await context.bot.send_invoice(
            chat_id=user_id,
            title="Покупка звезд",
            description=f"Пополнение баланса на {amount} ⭐ (свой размер)",
            payload=f"buy_stars_{amount}",
            provider_token="",
            currency="XTR",
            prices=prices,
        ...
        await update.message.reply_text(f"💳 Чек на {amount} ⭐ отправлен — оплатите его выше.")
    except Exception as e:
        logger.error(f"buy_stars_custom invoice error: {e}")
        await update.message.reply_text("❌ Не удалось создать чек. Попробуйте позже.")
    return MAIN_MENU


# ==================================
# === ПЛАТНЫЕ КНОПКИ-ТОВАРЫ (КАСТОМНЫЕ ПРОДУКТЫ) ===
# ==================================
# ПУНКТ 1 (платные кнопки): админ класса и разработчик могут создавать
# кнопки с ценой в звёздах. При нажатии такой кнопки бот динамически
# создаёт чек оплаты (Telegram Stars XTR) ровно на цену товара, а после
# успешной оплаты МГНОВЕННО доставляет содержимое (текст или ссылку).
# Оплата принимается и внутренними звёздами (с баланса), и через XTR.

def find_paid_button_by_id(button_id):
    """Ищет кнопку-товар по id среди глобальных и классных кастомных кнопок."""
    if not button_id:
        return None
    try:
        for btn in get_global_buttons():
            if btn.button_id == button_id:
                return btn
    except Exception:
        pass
    try:
        all_custom = load_data(CUSTOM_BUTTONS_FILE, {})
        for data in all_custom.values():
            try:
                if data.get('button_id') == button_id and data.get('is_active', True):
                    return CustomButton.from_dict(data)
            except Exception:
                continue
    except Exception:
        pass
...
async def deliver_paid_product(context, user, button):
    """Мгновенно выдаёт содержимое оплаченного товара."""
    display_name = button.name.replace("CLASS_", "") if (button.name or "").startswith("CLASS_") else button.name
    if button.button_type == "url":
        ...
        await context.bot.send_message(
            chat_id=user.user_id,
            text=f"🛍 Товар «{display_name}» оплачен и доставлен!\n\n🔗 Ссылка:",
            reply_markup=kb,
        )
    else:
        await context.bot.send_message(
...
async def offer_product_purchase(update, context, user, button):
    """Показывает экран покупки платной кнопки-товара.

    Пользователь платит внутренними звёздами (если хватает) либо
    Telegram Stars (динамический чек на цену товара).
    """
    price = int(getattr(button, 'price', 0) or 0)
    if price <= 0:
        return None
    display_name = button.name.replace("CLASS_", "") if (button.name or "").startswith("CLASS_") else button.name
    keyboard = []
    if int(getattr(user, 'stars_balance', 0) or 0) >= price:
        keyboard.append([InlineKeyboardButton(
            f"⭐ Оплатить с баланса ({price} ⭐)",
            callback_data=f"prod_pay_virt_{button.button_id}_{price}",
        )])
    keyboard.append([InlineKeyboardButton(
        f"💳 Оплатить Telegram Stars ({price} XTR)",
        callback_data=f"prod_pay_xtr_{button.button_id}_{price}",
    )])
    keyboard.append([InlineKeyboardButton("❌ Отмена", callback_data="cancel_action")])
    await update.message.reply_text(
        f"🛍 **Платная кнопка: {display_name}**\n\n"
        f"💵 Цена: {price} ⭐\n\n"
        f"Оплатите товар, и бот мгновенно выдаст его содержимое.",
...
async def prod_pay_virt_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Оплата товара внутренними звёздами + мгновенная доставка."""
    query = update.callback_query
    await query.answer()
    user_id = str(query.from_user.id)
    user = get_user(user_id)
    if not user:
        user = User(user_id)
    try:
        parts = query.data.split("_")  # prod_pay_virt_<btn_id>_<price>
        btn_id = parts[3]
        price = int(parts[4])
    except (ValueError, IndexError):
        await query.edit_message_text("Некорректный товар.")
        return MAIN_MENU
    button = find_paid_button_by_id(btn_id)
    if not button or int(getattr(button, 'price', 0) or 0) != price:
        await query.edit_message_text("❌ Товар не найден или цена изменилась.")
        return MAIN_MENU
    if int(getattr(user, 'stars_balance', 0) or 0) < price:
        await query.edit_message_text(
            f"Недостаточно звёзд на балансе ({user.stars_balance} ⭐ из {price} ⭐).\n"
            f"Купите звёзды и попробуйте снова.",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("💎 Купить звёзды", callback_data="buy_stars")],
                [InlineKeyboardButton("⬅️ В главное меню", callback_data="back_to_main")],
            ]),
        ...
        return MAIN_MENU
    # Списываем внутренние звёзды и сразу доставляем товар.
    add_stars_transaction(user_id, -price, f"Покупка товара «{button.name}»")
    try:
        await query.edit_message_text(f"✅ Оплата прошла! Доставляю товар…")
    except Exception:
        pass
    await deliver_paid_product(context, user, button)
    return MAIN_MENU


async def prod_pay_xtr_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """ПУНКТ 1: динамический чек Telegram Stars на цену товара."""
    query = update.callback_query
    await query.answer()
    user_id = str(query.from_user.id)
    try:
        parts = query.data.split("_")  # prod_pay_xtr_<btn_id>_<price>
        btn_id = parts[3]
        price = int(parts[4])
    except (ValueError, IndexError):
        await query.edit_message_text("Некорректный товар.")
        return MAIN_MENU
    button = find_paid_button_by_id(btn_id)
    if not button or int(getattr(button, 'price', 0) or 0) != price or price <= 0:
        await query.edit_message_text("❌ Товар не найден или цена изменилась.")
        return MAIN_MENU
    try:
        display_name = button.name.replace("CLASS_", "") if (button.name or "").startswith("CLASS_") else button.name
        prices = [LabeledPrice(label=f"Товар «{display_name}»", amount=price)]
        await context.bot.send_invoice(
            chat_id=user_id,
            title="Покупка товара",
            description=f"Кнопка-товар «{display_name}» ({price} ⭐)",
            payload=f"buy_product_{button.button_id}_{price}",
            provider_token="",
            currency="XTR",
            prices=prices,
            start_parameter="buy-product",
        )
        await query.edit_message_text(f"💳 Чек на {price} ⭐ отправлен — оплатите его выше.")
    except Exception as e:
        logger.error(f"prod_pay_xtr invoice error: {e}")
        await query.edit_message_text("❌ Не удалось создать чек. Попробуйте позже.")
    return MAIN_MENU
...
async def precheckout_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ПУНКТ 1 (автоматическая обработка платежей): все полезные нагрузки
    # валидируются перед подтверждением чека, чтобы Telegram не показывал
    # пользователю некорректные счета.
    if payload.startswith("buy_stars_"):
        # Динамическая сумма: 1..2500 XTR.
        try:
            amount = int(payload.split("_")[2])
            if 1 <= amount <= 2500:
                await query.answer(ok=True)
            else:
                await query.answer(ok=False, error_message="Некорректная сумма чека")
        except (ValueError, IndexError):
            await query.answer(ok=False, error_message="Некорректная сумма чека")
    elif payload.startswith("buy_product_"):
        # Покупка платной кнопки-товара: кнопка должна существовать, а
        # цена в чеке — совпадать с актуальной ценой кнопки.
        try:
            parts = payload.split("_")  # buy_product_<btn_id>_<price>
            btn_id = parts[2]
            pay_price = int(parts[3])
            button = find_paid_button_by_id(btn_id)
            if button and int(getattr(button, 'price', 0) or 0) == pay_price and pay_price > 0:
                await query.answer(ok=True)
            else:
                await query.answer(ok=False, error_message="Товар не найден или цена изменилась")
        except (ValueError, IndexError):
...
async def successful_payment_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if payload.startswith("buy_stars_"):
        ...
        # ПУНКТ 3 (статистика): покупка за реальные XTR учитывается в тратах.
        try:
            record_stars_purchase(user_id, PRICES.get('unblock', 40), "Разблокировка в классе (XTR)")
        except Exception:
            pass
        ...
        # ПУНКТ 3 (статистика): покупка за реальные XTR учитывается в тратах.
        try:
            unblock_price = int(payload.split("_")[2])
        except (ValueError, IndexError):
            unblock_price = PRICES.get('unblock', 40)
        record_stars_purchase(user_id, unblock_price, "Разблокировка (XTR)")
        ...
        try:
            ...
            amount = 0
        # ПУНКТ 3 (статистика): траты идут через единую функцию учёта.
        if amount > 0:
            record_stars_purchase(user_id, amount, "Покупка личной кнопки (XTR)")
        await update.message.reply_text(
        ...
        # ПУНКТ 3 (статистика): покупка за реальные XTR учитывается в тратах.
        try:
            vs_price = int(payload.split("_")[3])
        except (ValueError, IndexError):
            vs_price = PRICES.get('view_sender', 60)
        record_stars_purchase(user_id, vs_price, "Просмотр отправителя (XTR)")
        ...
        # ПУНКТ 1 (платные кнопки-товары): автоматическая обработка платежа
        # и мгновенное зачисление услуги — содержимое товара доставляется
        # сразу после оплаты.
        try:
            parts = payload.split("_")  # buy_product_<btn_id>_<price>
            btn_id = parts[2]
            prod_price = int(parts[3])
        except (ValueError, IndexError):
            prod_price = 0
            btn_id = None
        button = find_paid_button_by_id(btn_id) if btn_id else None
        if not button:
            await update.message.reply_text("❌ Товар не найден. Обратитесь в поддержку.")
            return MAIN_MENU
        if prod_price > 0:
            record_stars_purchase(user_id, prod_price, f"Покупка товара «{button.name}» (XTR)")
        try:
            await update.message.reply_text("✅ Оплата получена! Доставляю товар…")
        except Exception:
            pass
        await deliver_paid_product(context, user, button)
        return MAIN_MENU
        ...
        # ПУНКТ 3 (статистика): траты идут через единую функцию учёта.
        record_stars_purchase(user_id, amount, "Место под анонимки (XTR)")
        await update.message.reply_text(
...
async def stars_stats_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """ПУНКТ 3 (обновление статистики Stars).

    Показывает ТОЛЬКО общие агрегаты без подробной истории переводов:
    ...
    # Пересчитываем агрегаты из users.json — единственного источника правды.
    stats = _update_stars_aggregates()

    total_users = stats.get('total_users', 0)
    total_spent = stats.get('total_stars_spent', 0)
    top_donors = stats.get('top_donors', [])

    text = (
        f"📊 **Статистика звёзд**\n\n"
        f"👥 Пользователей: {total_users}\n"
        f"💸 Суммарно потрачено: {total_spent} ⭐\n\n"
        f"🏆 **Топ пользователей по тратам:**\n"
    )
    ...
    if top_donors:
        ...
        text += "Пока никто не тратил звёзды."
    ...
    try:
        ...
        await context.bot.send_message(
            ...
            text=text,
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=get_stars_keyboard(user),
        )
    return MAIN_MENU
...
PRICE_LABELS = {
    ...
    # ПУНКТ 2 (синхронизация цен): добавлены все ключи из PRICES, чтобы в
    # панели быстрого изменения цен не показывались «сырые» имена ключей.
    'anon_keep_month': '📨 Место под анонимки (мес)',
    'ai_generation': '🤖 Генерация DEVO+ai',
}
...
async def dev_global_button_url_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ПУНКТ 1 (платные кнопки): сначала цена, потом создание кнопки.
    context.user_data['global_button_content'] = url
    await update.message.reply_text(
        "💰 Введите цену глобальной кнопки в звёздах (0 или «бесплатно» — обычная кнопка):",
        reply_markup=get_cancel_keyboard(),
    )
    return DEV_GLOBAL_BUTTON_PRICE
...
async def dev_global_button_content_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ПУНКТ 1 (платные кнопки): сначала цена, потом создание кнопки.
    context.user_data['global_button_content'] = content
    await update.message.reply_text(
        "💰 Введите цену глобальной кнопки в звёздах (0 или «бесплатно» — обычная кнопка):",
        reply_markup=get_cancel_keyboard(),
    )
...
@timeout(CONVERSATION_TIMEOUT)
async def dev_global_button_price_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """ПУНКТ 1 (платные кнопки-товары): финальный шаг создания глобальной
    кнопки — ввод цены (0 — бесплатная) и сохранение с рассылкой всем."""
    user_id = str(update.effective_user.id)
    ...
    raw = (update.message.text or "").strip().lower().replace(" ", "").replace("⭐", "")
    if raw in ("0", "бесплатно", "free", "-"):
        price = 0
    else:
        try:
            price = int(raw)
            ...
            await update.message.reply_text(
                "Введите целое число (0 — бесплатная кнопка) или «бесплатно»:"
            )
            return DEV_GLOBAL_BUTTON_PRICE
        if price < 0 or price > 2500:
            await update.message.reply_text(
                "Цена должна быть от 0 до 2500 ⭐. Попробуйте ещё раз:"
            )
            return DEV_GLOBAL_BUTTON_PRICE

    button_name = context.user_data.get('global_button_name')
    button_type = context.user_data.get('global_button_type', 'text')
    content = context.user_data.pop('global_button_content', None)

    if not button_name or content is None:
        await update.message.reply_text("Данные не найдены. Начните создание заново.")
        return await developer_panel(update, context)

    button_id = generate_global_button_id()
    global_button = GlobalButton(button_id, button_name, content, button_type, price=price)

    buttons = load_global_buttons()
    buttons[button_id] = global_button
    save_global_buttons(buttons)

    price_note = f" ({price} ⭐)" if price > 0 else ""
    users = load_users()
    for uid, u in users.items():
        if not u.is_blocked and uid != user_id:
            try:
                ...
                await context.bot.send_message(
                    chat_id=uid,
                    text=f"🔔 Разработчик создал новую кнопку: *{button_name}*{price_note}",
                    parse_mode=ParseMode.MARKDOWN,
    ...
    if price > 0:
        await update.message.reply_text(
            f"✅ Платная глобальная кнопка-товар '{button_name}' создана! Цена: {price} ⭐"
        )
    else:
    ...
    context.user_data.pop('creating_global_button', None)
    context.user_data.pop('global_button_content', None)
    return await developer_panel(update, context)
...
async def save_bell_time_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if class_obj:
            ...
            # ИСПРАВЛЕНО (бесконечный цикл звонков): раньше после ввода времени
            # начала бот возвращал в ЭТО ЖЕ состояние EDIT_BELL_TIME, и
            # время окончания воспринималось как НОВОЕ начало — цикл длился
            # бесконечно. Теперь чёткий переход в EDIT_BELL_END: следующий
            # ввод попадёт в save_bell_end_handler.
...
async def save_bell_end_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # ИСПРАВЛЕНО (бесконечный цикл звонков): при ошибке формата ВРЕМЕНИ
        # ОКОНЧАНИЯ повторно спрашиваем именно окончание, а не начало.
        await update.message.reply_text("Введите время окончания в формате ЧЧ:ММ:")
        return EDIT_BELL_END
...
async def custom_button_url_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ПУНКТ 1 (платные кнопки): содержимое сохраняем во временное хранилище
    # и переходим к вводу цены — кнопка создаётся в custom_button_price_handler.
    context.user_data['custom_button_content'] = url
    await update.message.reply_text(
        "💰 Введите цену кнопки в звёздах (0 или «бесплатно» — обычная кнопка):\n\n"
        "Если цена больше нуля — кнопка станет платным товаром: ученики будут "
        "оплачивать её звёздами, а бот мгновенно выдаст содержимое после оплаты.",
        reply_markup=get_cancel_keyboard(),
    )
    return CUSTOM_BUTTON_PRICE
...
async def custom_button_content_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ПУНКТ 1 (платные кнопки): сначала спрашиваем цену, потом создаём.
    context.user_data['custom_button_content'] = content
    await update.message.reply_text(
        "💰 Введите цену кнопки в звёздах (0 или «бесплатно» — обычная кнопка):\n\n"
        "Если цена больше нуля — кнопка станет платным товаром: ученики будут "
        "оплачивать её звёздами, а бот мгновенно выдаст содержимое после оплаты.",
...
@timeout(CONVERSATION_TIMEOUT)
async def custom_button_price_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """ПУНКТ 1 (платные кнопки-товары): финальный шаг создания классной
    кнопки — ввод цены и сохранение (0 — бесплатная, как раньше)."""
    user_id = str(update.effective_user.id)
    user = get_user(user_id)
    if not user:
        user = User(user_id)
    ...
    button_name = context.user_data.get('custom_button_name')
    button_type = context.user_data.get('custom_button_type', 'text')
    content = context.user_data.pop('custom_button_content', None)
    class_code = context.user_data.get('current_admin_class')

    if not button_name or content is None or not class_code:
        await update.message.reply_text("Данные не найдены. Начните создание заново.")
        return await admin_panel(update, context)
        ...
        for member_id in class_obj.students + class_obj.admins:
            if member_id != str(user_id):
                try:
                    ...
                    await context.bot.send_message(
                        chat_id=member_id,
                        text=f"🔔 Администратор класса '{class_obj.class_name}' создал новую кнопку: *{button_display_name}*{price_note}",
                        parse_mode=ParseMode.MARKDOWN,
                        reply_markup=kb,
                    )
                except Exception as e:
    ...
    if price > 0:
        await update.message.reply_text(
            f"✅ Платная кнопка-товар '{button_name}' создана! Цена: {price} ⭐"
        )
    else:
    ...
    context.user_data.pop('custom_button_content', None)
...
# === ⚡ АВТОМАТИЗАЦИЯ (DeepSeek API) ===
# ==================================
# Управление ботом обычными фразами:
#   «русский на пятницу этой недели страница 45» → бот создаст ДЗ
#   на пятницу (дату посчитает сам DeepSeek по контексту);
#   «замени учителя по биологии, географии и обществознанию на Иванову»
#   → бот заменит учителей сразу по трём предметам.
#
# Как это работает:
#   1) Пользователь входит в режим кнопкой «⚡ Автоматизация» или /авто.
#   2) Каждое сообщение уходит в DeepSeek (api.deepseek.com), который
#      возвращает СТРОГИЙ JSON с распознанным действием.
#   3) Опасные (пишущие) действия показываются на подтверждение — бот
#      ничего не меняет молча.
#   4) После «✅ Выполнить» бот выполняет действие теми же функциями,
#      что и обычные кнопки админ-панели.
#
# Режим реализован как «оверлей» поверх основной FSM: обработчик живёт в
# группе -1 и перехватывает текст только пока включён флаг auto_mode,
# поэтому автоматизация работает В ЛЮБОМ состоянии бота, а после выхода
# пользователь возвращается ровно туда, где был.

import difflib

# Команды для выхода из режима автоматизации.
_AUTO_EXIT_TRIGGERS = {
    "❌ выйти из режима", "выйти из режима", "выход из режима",
    "❌ выйти", "выйти", "выход", "exit", "/exit", "stop", "/stop",
    "⬅️ назад в меню", "назад в меню", "back",
}

# Действия, доступные всем (только чтение).
_AUTO_READ_ONLY_ACTIONS = {
    "show_homework", "show_schedule", "show_teachers", "show_bells",
    "show_weather", "show_today", "show_tomorrow",
}

# Действия, требующие прав администратора класса (или разработчика).
_AUTO_ADMIN_ACTIONS = {
    "add_homework", "delete_homework", "replace_teacher", "add_subject",
    "set_bell", "set_schedule",
}

AUTOMATION_WELCOME_TEXT = (
    "⚡ **Режим автоматизации включён!**\n\n"
    "Теперь пишите мне обычными фразами — я сам создам ДЗ, заменю "
    "учителей, поставлю таймер, покажу расписание и погоду.\n\n"
    "Примеры:\n"
    "• «русский на пятницу этой недели страница 45»\n"
    "• «добавь дз по алгебре на завтра №120-125»\n"
    "• «замени учителя по биологии, географии и обществознанию на Иванову»\n"
    "• «поставь таймер на завтра 14:30 — встреча»\n"
    "• «покажи домашку на завтра»\n"
    "• «кто ведёт биологию?»\n\n"
    "Изменения выполняются только после вашего подтверждения.\n"
    "Выход — кнопка «❌ Выйти из режима» или команда /auto ещё раз."
)

AUTOMATION_EXAMPLES_TEXT = (
    "💡 **Примеры команд автоматизации:**\n\n"
    "📚 Домашнее задание:\n"
    "• «русский на пятницу этой недели страница 45»\n"
    "• «добавь дз по алгебре на завтра номера 120-125»\n"
    "• «удали дз по химии на завтра»\n\n"
    "👨‍🏫 Учителя:\n"
    "• «замени учителя по биологии, географии и обществознанию на Иванову»\n"
    "• «новый учитель по физике — Петров С.А.»\n"
    "• «добавь предмет экономика»\n\n"
    "⏰ Таймеры:\n"
    "• «напомни завтра в 14:30 — сборы»\n"
    "• «таймер на 2025-09-20 08:00 — день рождения»\n\n"
    "📋 Просмотр:\n"
    "• «покажи домашку на завтра / на пятницу»\n"
    "• «расписание на среду» · «какие звонки?» · «погода»\n\n"
    "🔔 Звонки (для админов):\n"
    "• «поменяй звонки 1 урока на 08:15-08:55»\n"
    "• «расписание на понедельник: 1. Математика, 2. Русский»"
)


def _is_class_admin_user(user, class_obj):
    """Разработчик или админ класса?"""
    if not user:
        return False
    if str(user.user_id) == str(DEVELOPER_ID):
        return True
    return bool(class_obj) and str(user.user_id) in class_obj.admins


def _fuzzy_match_subject(subjects, query):
    """Подбирает предмет класса по «человеческому» написанию.

    «биологии» → «Биология», «русский» → «Русский язык» и т. п.
    Возвращает точное имя предмета из списка или None.
    """
    if not query or not subjects:
        return None
    q = str(query).strip().lower().replace("ё", "е")
    if not q:
        return None
    norm = [str(s).strip().lower().replace("ё", "е") for s in subjects]
    # 1) Точное совпадение.
    for s, n in zip(subjects, norm):
        if n == q:
            return s
    # 2) Начало слова.
    for s, n in zip(subjects, norm):
    ...
    for s, n in zip(subjects, norm):
        if q in n or n in q:
            return s
    # 4) Нечёткое сравнение (учитывает падежи: «биологии» ↔ «биология»).
    close = difflib.get_close_matches(q, norm, n=1, cutoff=0.55)
    if close:
        for s, n in zip(subjects, norm):
            if n == close[0]:
                return s
...
def _automation_provider():
    """Какой LLM-провайдер использовать для автоматики.

    По умолчанию — DeepSeek API (если задан DEEPSEEK_API_KEY).
    Если ключа DeepSeek нет, но есть Groq — временно работаем через Groq,
    чтобы функция не «умирала» молча (об этом пишет предупреждение в лог).
    Возвращает "deepseek", "groq" или None (ничего не настроено).
    """
    if AUTOMATION_PROVIDER == "groq":
...
def _extract_json_object(text):
    """Достаёт JSON-объект из ответа модели (умеет срезать ```json ...```)."""
    if not text:
    ...
    try:
        ...
        pass
    # Пытаемся вырезать { ... } по внешним скобкам.
    start = cleaned.find("{")
    end = cleaned.rfind("}")
    if start != -1 and end > start:
        ...
        try:
            ...
            # Последняя попытка: убрать висячие запятые.
            import re as _re
...
class AutomationUnavailableError(RuntimeError):
    """Автоматизация не настроена (нет ключа провайдера)."""


async def _automation_llm_json(system_prompt, user_prompt):
    """Один запрос к LLM (DeepSeek, фолбэк Groq) → распарсенный JSON."""
    provider = _automation_provider()
    messages = [
        ...
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=45)) as session:
            async with session.post(url, headers=headers, json=payload) as resp:
                if resp.status != 200:
                    ...
                    raise RuntimeError(f"DeepSeek API HTTP {resp.status}: {body[:200]}")
                data = await resp.json()
        try:
            content = data["choices"][0]["message"]["content"]
        except (KeyError, IndexError, TypeError) as e:
            raise RuntimeError(f"неожиданный ответ DeepSeek: {e}")
    ...
    if provider == "groq":
        ...
        return _extract_json_object(chat_completion.choices[0].message.content)
    raise AutomationUnavailableError(
        "DEEPSEEK_API_KEY не задан — задайте его в настройках хостинга "
        "(https://platform.deepseek.com), чтобы включить автоматизацию."
    )


def _build_automation_system_prompt(user, class_obj):
    """Собирает системный промпт с контекстом пользователя и класса."""
    local_now = get_local_time(user)
    days_ru = ["Понедельник", "Вторник", "Среда", "Четверг", "Пятница", "Суббота", "Воскресенье"]
    today_str = local_now.strftime("%Y-%m-%d")
    weekday = days_ru[local_now.weekday()]
    is_admin = _is_class_admin_user(user, class_obj)
    subjects = list(class_obj.subjects) if class_obj else []
    teachers = dict(class_obj.teachers) if class_obj else {}
    role = "администратор класса" if is_admin else "ученик"
    if class_obj:
        subjects_line = json.dumps(subjects, ensure_ascii=False)
        teachers_line = json.dumps(teachers, ensure_ascii=False)
    else:
        subjects_line = "[] (пользователь не состоит в классе)"
        teachers_line = "{} (пользователь не состоит в классе)"
    return (
        "Ты — встроенный парсер команд школьного Telegram-бота DEVO+WORKS. "
        "Твоя единственная задача — превратить сообщение пользователя в JSON-объект с намерением. "
        "Никаких пояснений — только валидный JSON.\n\n"
        f"КОНТЕКСТ:\n"
        f"- Сегодняшняя дата (локальное время пользователя): {today_str} ({weekday})\n"
        f"- Роль пользователя: {role}\n"
        f"- Предметы класса: {subjects_line}\n"
        f"- Учителя (предмет → имя): {teachers_line}\n"
        f"- Дни недели: Понедельник, Вторник, Среда, Четверг, Пятница, Суббота, Воскресенье\n\n"
        "ВОЗМОЖНЫЕ ДЕЙСТВИЯ (поле \"action\"):\n"
        "1. \"add_homework\" — добавить ДЗ. Поля: \"subject\" (строка), \"text\" (строка), \"date\" (YYYY-MM-DD).\n"
        "2. \"delete_homework\" — удалить ДЗ. Поля: \"subject\", \"date\" (YYYY-MM-DD или null).\n"
        "3. \"replace_teacher\" — заменить/назначить учителя. Поля: \"subjects\" (массив строк), \"teacher\" (строка).\n"
        "4. \"add_subject\" — добавить предмет. Поля: \"subject\" (строка).\n"
        "5. \"set_bell\" — изменить звонок урока. Поля: \"lesson\" (номер урока строкой, \"1\"..\"8\"), \"start\" (ЧЧ:ММ), \"end\" (ЧЧ:ММ).\n"
        "6. \"set_schedule\" — заменить расписание дня. Поля: \"day\" (день недели), \"content\" (текст расписания).\n"
        "7. \"add_timer\" — поставить напоминание. Поля: \"date\" (YYYY-MM-DD), \"time\" (ЧЧ:ММ), \"text\" (текст напоминания; если не указан — \"Напоминание\").\n"
        "8. \"show_homework\" — показать ДЗ. Поле \"date\" (YYYY-MM-DD или null — всё ДЗ).\n"
        "9. \"show_schedule\" — показать расписание. Поле \"day\" (день недели или null — вся неделя).\n"
        "10. \"show_teachers\" — показать учителей.\n"
        "11. \"show_bells\" — показать звонки.\n"
        "12. \"show_weather\" — показать погоду.\n"
        "13. \"show_today\" — расписание и ДЗ на сегодня.\n"
        "14. \"show_tomorrow\" — расписание и ДЗ на завтра.\n"
        "15. \"unknown\" — не смог понять запрос (обычная болтовня, не команда).\n\n"
        "ПРАВИЛА:\n"
        "- Отвечай ТОЛЬКО валидным JSON-объектом, без markdown-обёрток и без лишнего текста.\n"
        "- Все даты вычисляй относительно сегодняшней даты из контекста, формат строго YYYY-MM-DD. "
        "Даты — только сегодня или будущее: «пятница этой недели» = ближайшая пятница, начиная с сегодня; "
        "«завтра» = сегодня+1; «послезавтра» = сегодня+2; «на следующей неделе» = +7 дней.\n"
        "- Названия предметов сопоставляй с предметами класса из контекста "
        "(«русский» → «Русский язык», «по биологии» → «Биология»). "
        "Для add_homework, если предмета нет в списке — верни название, как написал пользователь.\n"
        "- «замени учителя по биологии, географии и обществознанию на Иванову» → "
        "\"subjects\": [\"Биология\", \"География\", \"Обществознание\"], \"teacher\": \"Иванова\".\n"
        "- Не выдумывай поля. Если данных не хватает — верни {\"action\": \"unknown\"}.\n"
        "- Пиши строки на русском так, как их написал пользователь (не переводи)."
    )


async def parse_automation_intent(text_in, user, class_obj):
    """Превращает фразу пользователя в JSON-намерение (через DeepSeek)."""
    system_prompt = _build_automation_system_prompt(user, class_obj)
    result = await _automation_llm_json(system_prompt, text_in)
...
def _validate_weekday(value):
    """Нормализует написание дня недели: «пятницу»/«Пн»/«в среду» → «Пятница»."""
    days = ["Понедельник", "Вторник", "Среда", "Четверг", "Пятница", "Суббота", "Воскресенье"]
    short_days = ["пн", "вт", "ср", "чт", "пт", "сб", "вс"]
    if not value:
        return None
    v = str(value).strip().lower().replace("ё", "е").strip(".,!? ")
    # Убираем предлоги: «в среду», «на пятницу».
    for prefix in ("на ", "в "):
    ...
    for d in days:
        dl = d.lower()
        # Полная форма и падежи: «пятницу» ~ «пятница» (по первым 4 символам).
        if v == dl or v.startswith(dl) or (len(v) >= 3 and dl.startswith(v[:4])):
            return d
    for i, sd in enumerate(short_days):
        if v == sd or v.startswith(sd + "."):
            return days[i]
    return None


def format_auto_intent_preview(intent, user, class_obj):
    """Человекочитаемое описание действия для подтверждения."""
    action = intent.get("action")
    if action == "add_homework":
        return (
            "📚 **Создать домашнее задание:**\n\n"
            f"Предмет: {intent.get('subject', '?')}\n"
            f"Дата: {intent.get('date', '?')}\n"
            f"Задание: {intent.get('text', '?')}\n\n"
            "Выполнить?"
        )
    if action == "delete_homework":
        date_note = f"Дата: {intent.get('date')}" if intent.get('date') else "Дата: любая (все задания по предмету)"
        return (
    ...
    if action == "replace_teacher":
        ...
        if isinstance(subjects, str):
            subjects = [subjects]
        subjects_note = ", ".join(subjects) if subjects else "?"
        old = []
        if class_obj:
            for s in subjects:
                matched = _fuzzy_match_subject(list(class_obj.teachers.keys()), s)
                old.append(f"{matched or s}: {class_obj.teachers.get(matched, '—')}")
        old_note = "\n".join(f"   • {o}" for o in old) if old else "   • (нет данных)"
        return (
            "👨‍🏫 **Заменить учителя(ей):**\n\n"
            f"Предметы: {subjects_note}\n"
            f"Новый учитель: {intent.get('teacher', '?')}\n\n"
            f"Сейчас:\n{old_note}\n\n"
            "Выполнить?"
        )
    ...
    if action == "set_bell":
        return (
            "🔔 **Изменить звонок:**\n\n"
            f"Урок: {intent.get('lesson', '?')}\n"
            f"Начало: {intent.get('start', '?')}\n"
            f"Окончание: {intent.get('end', '?')}\n\n"
            "Выполнить?"
        )
    if action == "set_schedule":
        return (
            "📅 **Изменить расписание:**\n\n"
            f"День: {intent.get('day', '?')}\n"
            f"Новое расписание:\n{intent.get('content', '?')}\n\n"
            "Выполнить?"
        )
    if action == "add_timer":
        return (
            "⏰ **Поставить таймер:**\n\n"
            f"Дата: {intent.get('date', '?')}\n"
            f"Время: {intent.get('time', '?')}\n"
            f"Текст: {intent.get('text', 'Напоминание')}\n\n"
            "Выполнить?"
        )
    return "Выполнить?"


async def _send_long_reply(update, text, parse_mode=None, context=None, chat_id=None):
    """Отправляет текст, разбивая на куски по 4000 символов.

    Работает и из message-обновлений, и из callback-обновлений (в последнем
    случае передайте context и chat_id)."""
    if not text:
        return
    ...
    async def _send(chunk):
        ...
        if msg is not None:
            try:
                ...
                logger.error(f"_send_long_reply: {e}")
    ...
    for i in range(0, len(text), chunk_size):
        ok = await _send(text[i:i + chunk_size])
        if not ok:
            return


async def execute_auto_intent(update, context, intent, user, class_obj):
    """Выполняет распознанное намерение. Возвращает текст результата.

    Для пишущих действий проверяет права администратора класса
    (или разработчика). Бросает PermissionError при нехватке прав.
    """
    ...
    # ---------- Пишущие действия ----------
    if action in _AUTO_ADMIN_ACTIONS:
        if not is_admin:
            raise PermissionError(
                "Это действие доступно только администраторам класса "
                "(или разработчику)."
            )
        if not class_obj:
            return "⚠️ Вы не состоите в классе — создайте или войдите в класс через «🎓 Управление классами»."

        if action == "add_homework":
            subject_raw = str(intent.get("subject") or "").strip()
            hw_text = str(intent.get("text") or "").strip()
            date_str = _validate_date(intent.get("date"))
            if not subject_raw or not hw_text or not date_str:
                return "⚠️ Не хватает данных (предмет / задание / дата). Уточните фразу и попробуйте снова."
            subjects = list(class_obj.subjects or [])
            matched = _fuzzy_match_subject(subjects, subject_raw)
            is_new_subject = matched is None
            subject = matched or subject_raw
            if is_new_subject and subject not in subjects:
                subjects.append(subject)
                class_obj.subjects = subjects
            if subject not in class_obj.homework:
                class_obj.homework[subject] = []
            class_obj.homework[subject].append({
                'text': hw_text,
            ...
            classes[class_obj.class_code] = class_obj
            save_classes(classes)
            new_note = " (новый предмет добавлен)" if is_new_subject else ""
            return (
                f"✅ Домашнее задание добавлено{new_note}!\n\n"
                f"📅 Дата: {date_str}\n📚 Предмет: {subject}\n📝 Задание: {hw_text}"
            )

        if action == "delete_homework":
            subject_raw = str(intent.get("subject") or "").strip()
            matched = _fuzzy_match_subject(list(class_obj.homework.keys()), subject_raw) \
                or _fuzzy_match_subject(list(class_obj.subjects or []), subject_raw)
            if not matched:
                return f"⚠️ Предмет «{subject_raw}» не найден в ДЗ класса."
            date_str = _validate_date(intent.get("date")) if intent.get("date") else None
            entries = class_obj.homework.get(matched, [])
            if date_str:
                to_keep = [a for a in entries if a.get('date') != date_str]
                removed = len(entries) - len(to_keep)
            else:
                to_keep = []
                removed = len(entries)
            if removed == 0:
                return f"⚠️ По предмету «{matched}» нет подходящих заданий для удаления."
            class_obj.homework[matched] = to_keep
            classes = load_classes()
            classes[class_obj.class_code] = class_obj
            save_classes(classes)
            date_note = f" на {date_str}" if date_str else ""
            return f"✅ Удалено заданий: {removed} по предмету «{matched}»{date_note}."

        if action == "replace_teacher":
            subjects_in = intent.get("subjects") or []
            if isinstance(subjects_in, str):
                subjects_in = [subjects_in]
            teacher_name = str(intent.get("teacher") or "").strip()
            if not subjects_in or not teacher_name:
                return "⚠️ Не хватает данных (предметы / имя учителя)."
            teachers_keys = list(class_obj.teachers.keys())
            existing_subjects = list(class_obj.subjects or [])
            results = []
            for s in subjects_in:
                s = str(s).strip()
                if not s:
                    continue
                matched = _fuzzy_match_subject(teachers_keys, s)
                if matched:
                    results.append((matched, class_obj.teachers.get(matched, "Не указан"), False))
                else:
                    # Новый предмет + новый учитель (как в «➕ Добавить предмет»).
                    results.append((s, "Не указан", True))
            if not results:
                return "⚠️ Не удалось распознать предметы."
            for subj, _old, is_new in results:
                class_obj.teachers[subj] = teacher_name
                if is_new and subj not in existing_subjects:
                    class_obj.subjects = existing_subjects + [subj]
                    existing_subjects = class_obj.subjects
            classes = load_classes()
            classes[class_obj.class_code] = class_obj
            save_classes(classes)
            lines = []
            for subj, old, is_new in results:
                lines.append(f"📚 {subj}: {old} → {teacher_name}" + (" (новый предмет)" if is_new else ""))
            return "✅ Учителя обновлены:\n" + "\n".join(lines)

        if action == "add_subject":
            subject = str(intent.get("subject") or "").strip()
            if not subject:
                return "⚠️ Не указан предмет."
            if subject in (class_obj.subjects or []):
                return f"ℹ️ Предмет «{subject}» уже есть в классе."
            class_obj.subjects = (class_obj.subjects or []) + [subject]
            class_obj.teachers[subject] = "Не указан"
            classes = load_classes()
            classes[class_obj.class_code] = class_obj
            save_classes(classes)
            return f"✅ Предмет «{subject}» добавлен (учитель: Не указан)."

        if action == "set_bell":
            lesson = str(intent.get("lesson") or "").strip()
            start = _validate_hhmm(intent.get("start"))
            end = _validate_hhmm(intent.get("end"))
            if not lesson or not start or not end:
                return "⚠️ Нужны номер урока и оба времени в формате ЧЧ:ММ (например, 08:15 и 08:55)."
            if lesson not in class_obj.bells:
                # Номер урока «6»/«06» — нормализуем к существующим ключам.
                found = None
                for k in class_obj.bells.keys():
                    if str(k).lstrip("0") == lesson.lstrip("0"):
                        found = k
                        break
                if found is None:
                    return f"⚠️ Урок «{lesson}» не найден. Доступные уроки: {', '.join(class_obj.bells.keys())}."
                lesson = found
            class_obj.bells[lesson]['start'] = start
            class_obj.bells[lesson]['end'] = end
            classes = load_classes()
            classes[class_obj.class_code] = class_obj
            save_classes(classes)
            return f"✅ Звонок {lesson} урока обновлён: {start} - {end}"

        if action == "set_schedule":
            day = _validate_weekday(intent.get("day"))
            content = str(intent.get("content") or "").strip()
            if not day or not content:
                return "⚠️ Нужны день недели и текст расписания."
    ...
    # ---------- Таймер (доступен всем) ----------
    if action == "add_timer":
        date_str = _validate_date(intent.get("date"))
        time_str = _validate_hhmm(intent.get("time"))
        text = str(intent.get("text") or "Напоминание").strip() or "Напоминание"
        if not date_str or not time_str:
            return "⚠️ Нужны дата (ГГГГ-ММ-ДД) и время (ЧЧ:ММ) таймера."
        timers = load_data(TIMERS_FILE, {})
        timer_id = generate_timer_id()
        timers[timer_id] = {
            'user_id': user_id,
        ...
        try:
            ...
            logger.error(f"Автоматизация: не удалось запланировать таймер {timer_id}: {e}")
        return (
            f"✅ Таймер установлен!\n\n📅 {date_str} в {time_str}\n📝 {text}\n\n"
            f"Бот пришлёт напоминание в указанное время."
        )

    # ---------- Действия просмотра ----------
    if action in _AUTO_READ_ONLY_ACTIONS:
        if action in ("show_homework", "show_schedule", "show_teachers", "show_bells", "show_today", "show_tomorrow") and not class_obj:
            return "⚠️ Вы не состоите в классе — создайте или войдите в класс через «🎓 Управление классами»."

        if action == "show_homework":
            date_str = _validate_date(intent.get("date")) if intent.get("date") else None
            if date_str:
                hw = get_homework_for_date(class_obj, date_str)
                if not hw:
                    return f"📝 Домашнее задание на {date_str} не задано."
                return format_homework(hw, date_str)
            if not class_obj.homework:
                return "📝 Домашнее задание не задано."
            return format_homework(class_obj.homework)

        if action == "show_schedule":
            day = _validate_weekday(intent.get("day")) if intent.get("day") else None
            if day:
                return f"📅 {day}:\n\n{get_day_schedule(class_obj, day)}"
            parts = [f"📅 Расписание на неделю — {class_obj.class_name}\n"]
            for d in ["Понедельник", "Вторник", "Среда", "Четверг", "Пятница", "Суббота", "Воскресенье"]:
                parts.append(f"\n📌 {d}:\n{get_day_schedule(class_obj, d)}")
            return "\n".join(parts)

        if action == "show_teachers":
            if not class_obj.teachers:
                return "👨‍🏫 Учителя ещё не указаны."
            lines = [f"👨‍🏫 **{s}:** {t}" for s, t in class_obj.teachers.items()]
            return "\n".join(lines)

        if action == "show_bells":
            return get_bells_info(class_obj, user)

        if action == "show_weather":
            if not getattr(user, 'city', None):
                return "🏙 Город не указан. Установите его в ⚙️ Настройки → 🌦 Настройки погоды."
            return await weather_current_text(user.city)

        if action == "show_today":
            local_now = get_local_time(user)
            day_name = get_day_name(local_now.weekday())
            today_str = local_now.strftime("%Y-%m-%d")
            message = f"📅 Расписание на сегодня ({day_name}, {today_str}):\n\n{get_day_schedule(class_obj, day_name)}"
            hw = get_homework_for_date(class_obj, today_str)
            if hw:
                message += "\n\n" + format_homework(hw, "сегодня").replace("**", "")
            else:
                message += "\n\n📝 Домашнее задание на сегодня не задано."
            return message
        ...
        if action == "show_tomorrow":
            ...
            tomorrow_str = tomorrow.strftime("%Y-%m-%d")
            message = f"📅 Расписание на завтра ({day_name}, {tomorrow_str}):\n\n{get_day_schedule(class_obj, day_name)}"
            hw = get_homework_for_date(class_obj, tomorrow_str)
            if hw:
                message += "\n\n" + format_homework(hw, "завтра").replace("**", "")
            else:
                message += "\n\n📝 Домашнее задание на завтра не задано."
            return message
...
async def enter_automation_mode(update, context, user):
    """Включает режим автоматизации (кнопка меню или /авто)."""
    context.user_data['ai_mode'] = False
    context.user_data['messaging_admin'] = False
    context.user_data['replying_to_anon'] = False
    context.user_data['auto_mode'] = True
    reply_kb = ReplyKeyboardMarkup(
        [["❌ Выйти из режима", "💡 Примеры"]],
        resize_keyboard=True,
    )
    target = update.message if getattr(update, "message", None) else None
    if target is None and getattr(update, "callback_query", None):
        await update.callback_query.answer()
        try:
            await update.callback_query.edit_message_text("⚡ Режим автоматизации включён (смотрите новое сообщение).")
        except Exception:
            pass
    try:
...
def _exit_automation_mode(update, context, user):
    context.user_data['auto_mode'] = False
    context.user_data.pop('pending_auto_intent', None)


async def _automation_process(update, context):
    """Основная логика обработки сообщения в режиме автоматизации."""
    user_id = str(update.effective_user.id)
    user = get_user(user_id)
    if not user or not user.setup_completed:
        context.user_data['auto_mode'] = False
    ...
    # Выход из режима.
    if lowered in _AUTO_EXIT_TRIGGERS or text_in in _AUTO_EXIT_TRIGGERS:
        _exit_automation_mode(update, context, user)
        await update.message.reply_text(
            "👋 Вы вышли из режима автоматизации.",
            reply_markup=get_main_menu_keyboard(user),
        )
        return

    # Примеры команд.
    if lowered in ("💡 примеры", "примеры", "пример", "помощь", "help", "команды"):
        await update.message.reply_text(AUTOMATION_EXAMPLES_TEXT, parse_mode=ParseMode.MARKDOWN)
        return
    ...
    # Антиспам: не чаще одного запроса к модели раз в 2 секунды.
    if is_user_spamming(user_id, "automation", min_interval=2.0, burst=3, burst_window=10.0):
        await update.message.reply_text("⏳ Слишком быстро. Подождите пару секунд.")
        return

    status = None
    try:
        status = await update.message.reply_text("🧠 Распознаю команду…")
    except Exception:
        pass

    try:
        intent = await parse_automation_intent(text_in, user, class_obj)
    except AutomationUnavailableError as e:
        if status:
            try:
        ...
        logger.error(f"parse_automation_intent: {e}")
        err_text = "⚠️ Не удалось распознать команду (сервис ИИ недоступен). Попробуйте позже."
        if status:
            try:
    ...
    action = (intent or {}).get("action") or "unknown"

    if action in (None, "", "unknown", "help"):
        reply = (
            "🤔 Не понял команду. Вот что я умею:\n\n"
            + AUTOMATION_EXAMPLES_TEXT
        )
        if status:
            try:
                ...
                await _send_long_reply(update, reply, ParseMode.MARKDOWN)
        else:
            await _send_long_reply(update, reply, ParseMode.MARKDOWN)
        return

    # Действия просмотра выполняем сразу, без подтверждения.
    if action in _AUTO_READ_ONLY_ACTIONS:
        try:
            result = await execute_auto_intent(update, context, intent, user, class_obj)
        except Exception as e:
            logger.error(f"execute_auto_intent (read) {action}: {e}")
            result = "⚠️ Не удалось выполнить запрос. Попробуйте позже."
        if status:
            try:
                await status.edit_text(result[:4000])
            except Exception:
                await _send_long_reply(update, result)
        else:
            await _send_long_reply(update, result)
        return

    # Пишущие действия и таймер — сначала подтверждение.
    if action in _AUTO_ADMIN_ACTIONS or action == "add_timer":
        if action in _AUTO_ADMIN_ACTIONS and not _is_class_admin_user(user, class_obj):
            notice = (
                "⛔ Это действие доступно только администраторам класса "
                "(или разработчику)."
            )
            if status:
                try:
                    await status.edit_text(notice)
                except Exception:
                    pass
            else:
                await update.message.reply_text(notice)
            return
        preview = format_auto_intent_preview(intent, user, class_obj)
        context.user_data['pending_auto_intent'] = intent
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ Выполнить", callback_data="auto_exec_yes")],
            [InlineKeyboardButton("❌ Отмена", callback_data="auto_exec_no")],
        ])
        sent = False
        if status:
            try:
                await status.edit_text(preview, parse_mode=ParseMode.MARKDOWN, reply_markup=keyboard)
                sent = True
            except Exception:
                try:
                    await status.edit_text(preview, reply_markup=keyboard)
                    sent = True
                except Exception:
                    pass
        if not sent:
            try:
                ...
                try:
                    ...
                    logger.error(f"automation preview send: {e}")
        return

    # На всякий случай — неизвестное действие.
    if status:
        try:
            await status.edit_text("🤔 Неизвестное действие. Попробуйте переформулировать.")
        except Exception:
            pass
    else:
        await update.message.reply_text("🤔 Неизвестное действие. Попробуйте переформулировать.")


async def automation_gate_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """«Оверлей» режима автоматизации (группа -1).

    Если auto_mode выключен — просто возвращаем управление (сообщение
    обработает основная FSM в группе 0). Если включён — обрабатываем фразу
    и поднимаем ApplicationHandlerStop, чтобы сообщение не ушло в
    обработчики состояния (например, в ANONYMOUS_SEND_MESSAGE).
    """
    if not context.user_data.get('auto_mode'):
        return
    try:
        await _automation_process(update, context)
    except Exception as e:
        logger.error(f"automation_gate_handler: {e}")
        try:
            await update.message.reply_text(
                "⚠️ Ошибка автоматики. Попробуйте ещё раз или выйдите из режима."
            )
        except Exception:
            pass
...
async def auto_exec_yes_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Подтверждение выполнения действия автоматизации."""
    query = update.callback_query
    await query.answer()
    user_id = str(query.from_user.id)
    user = get_user(user_id)
    if not user:
        user = User(user_id)
    intent = context.user_data.pop('pending_auto_intent', None)
    if not intent:
        try:
            await query.edit_message_text(
                "⚠️ Команда не найдена — возможно, уже выполнена или отменена. "
                "Введите её заново в режиме автоматизации."
            )
        except Exception:
            pass
        return MAIN_MENU
    ...
    try:
        ...
        logger.error(f"auto_exec_yes: {e}")
        result = "❌ Не удалось выполнить действие. Попробуйте позже."
    try:
        if len(result) <= 4000:
            ...
            for i in range(0, len(rest), 4000):
                await context.bot.send_message(chat_id=user_id, text=rest[i:i + 4000])
    except Exception:
        await _send_long_reply(update, result, context=context, chat_id=user_id)
    return MAIN_MENU


async def auto_exec_no_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Отмена выполнения действия автоматизации."""
    query = update.callback_query
    await query.answer()
    context.user_data.pop('pending_auto_intent', None)
    try:
        await query.edit_message_text("❌ Отменено. Введите другую команду или выйдите из режима.")
    except Exception:
        pass
    return MAIN_MENU


async def auto_examples_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Inline-кнопка «💡 Примеры» в режиме автоматизации."""
    query = update.callback_query
    await query.answer()
    try:
        await query.edit_message_text(AUTOMATION_EXAMPLES_TEXT, parse_mode=ParseMode.MARKDOWN)
    except Exception:
        await context.bot.send_message(
            chat_id=query.from_user.id,
            text=AUTOMATION_EXAMPLES_TEXT,
            parse_mode=ParseMode.MARKDOWN,
        )
    return MAIN_MENU


async def auto_exit_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Inline-кнопка выхода из режима автоматизации."""
    query = update.callback_query
    await query.answer()
    user_id = str(query.from_user.id)
    user = get_user(user_id)
    ...
    context.user_data.pop('pending_auto_intent', None)
    try:
        await query.edit_message_text("👋 Вы вышли из режима автоматизации.")
    except Exception:
        pass
    try:
        await context.bot.send_message(
...
# ==================================
# === ⌨️ БЫСТРЫЕ КОМАНДЫ (работают везде) ===
# ==================================
# Зарегистрированы в группе -1, ВЫШЕ основной FSM, поэтому срабатывают из
# любого состояния бота (во время ввода ДЗ, в админке, в чате с AI…) и не
# ломают текущее состояние. Старые функции не затронуты: все кнопки меню
# продолжают работать как раньше.

def _parse_user_date_arg(arg, user):
    """Парсит аргумент-дату для /dz: «завтра», «сегодня», 2025-09-15, 15.09."""
    a = (arg or "").strip().lower()
    if not a:
        return None
    if a in ("сегодня", "today", "тудей"):
        return get_local_time(user).strftime("%Y-%m-%d")
    if a in ("завтра", "tomorrow"):
        return (get_local_time(user) + timedelta(days=1)).strftime("%Y-%m-%d")
    if a in ("послезавтра"):
        return (get_local_time(user) + timedelta(days=2)).strftime("%Y-%m-%d")
    for fmt in ("%Y-%m-%d", "%d.%m.%Y", "%d.%m"):
        try:
            d = datetime.strptime(a, fmt)
            if fmt == "%d.%m":
                d = d.replace(year=get_local_time(user).year)
            return d.strftime("%Y-%m-%d")
        except ValueError:
            continue
    return "INVALID"
...
async def _qc_require_user(update, context):
    """Общие проверки для быстрых команд. Возвращает (user, class_obj) или None."""
    user_id = str(update.effective_user.id)
    user = get_user(user_id)
    if not user or not user.setup_completed:
        await update.message.reply_text(
            "👋 Сначала зарегистрируйтесь через /start — потом команды заработают."
        )
        return None
    if not await ensure_subscribed(update, context):
        return None
    ...
    if class_obj and is_user_class_blocked(user_id, class_obj.class_code):
        await update.message.reply_text("Вы заблокированы в этом классе.")
        return None
    return user, class_obj


def _cmd_text_args(update):
    """Аргументы команды из текста сообщения: «/дз завтра» → «завтра».

    Используется вместо context.args, потому что кириллические команды
    (/дз, /учителя …) ловятся regex-диспетчером, а не CommandHandler.
    """
...
# Кириллические быстрые команды: PTB не позволяет регистрировать их через
# CommandHandler (только [a-z0-9_]), поэтому ловим regex-диспетчером.
_QUICK_CMD_REGEX = (
    r"^/(?:дз|учителя|расписание|сегодня|завтра|звонки|погода|таймер|"
    r"зв[её]зд[ыа]|авто)(?:@[\w]+)?(?:\s|$)"
)


async def quick_command_dispatcher(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Маршрутизатор кириллических быстрых команд (группа -1)."""
    text = (update.message.text or "").strip().lower()
    cmd = text.split("@")[0].split(maxsplit=1)[0] if text else ""
    mapping = {
        "/дз": qc_homework,
        "/учителя": qc_teachers,
        "/расписание": qc_schedule,
        "/сегодня": qc_today,
        "/завтра": qc_tomorrow,
        "/звонки": qc_bells,
        "/погода": qc_weather,
...
async def qc_homework(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/dz, /дз, /homework [дата] — домашнее задание (всё или на дату)."""
    res = await _qc_require_user(update, context)
    if not res:
        return
    user, class_obj = res
    if not class_obj:
        await update.message.reply_text("Вы не состоите в классе.")
        return
    args = _cmd_text_args(update)
    if args:
        date_str = _parse_user_date_arg(args, user)
        if date_str == "INVALID" or date_str is None:
            await update.message.reply_text(
                "Не понял дату. Примеры: /dz завтра · /dz 2025-09-15 · /dz 15.09"
            )
            return
        hw = get_homework_for_date(class_obj, date_str)
        if not hw:
            await update.message.reply_text(f"📝 Домашнее задание на {date_str} не задано.")
            return
        await _send_long_reply(update, format_homework(hw, date_str), "Markdown")
        return
    # Без даты — всё ДЗ (как кнопка «📝 Домашнее задание»).
    if not class_obj.homework:
        await update.message.reply_text("📝 Домашнее задание не задано.")
        return
    hw_text = format_homework(class_obj.homework)
    await _send_long_reply(update, hw_text, "Markdown")


async def qc_teachers(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/teachers, /учителя — список учителей класса."""
    res = await _qc_require_user(update, context)
    if not res:
        return
    user, class_obj = res
    if not class_obj or not class_obj.teachers:
        await update.message.reply_text("👨‍🏫 Учителя ещё не указаны.")
        return
    lines = [f"👨‍🏫 **{s}:** {t}" for s, t in class_obj.teachers.items()]
    await _send_long_reply(update, "\n".join(lines), "Markdown")


async def qc_schedule(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/schedule, /расписание — расписание на неделю."""
    res = await _qc_require_user(update, context)
    if not res:
        return
...
async def _qc_day_view(update, user, class_obj, offset_days):
    local_now = get_local_time(user) + timedelta(days=offset_days)
    day_name = get_day_name(local_now.weekday())
    date_str = local_now.strftime("%Y-%m-%d")
    word = "сегодня" if offset_days == 0 else "завтра"
    message = f"📅 Расписание на {word} ({day_name}, {date_str}):\n\n{get_day_schedule(class_obj, day_name)}"
    hw = get_homework_for_date(class_obj, date_str)
    if hw:
        message += "\n\n" + format_homework(hw, word).replace("**", "")
    else:
        message += f"\n\n📝 Домашнее задание на {word} не задано."
    await _send_long_reply(update, message)


async def qc_today(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not class_obj:
        await update.message.reply_text("Вы не состоите в классе.")
        return
    await _qc_day_view(update, user, class_obj, 0)


async def qc_tomorrow(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not class_obj:
        await update.message.reply_text("Вы не состоите в классе.")
        return
    await _qc_day_view(update, user, class_obj, 1)


async def qc_bells(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/bells, /звонки — расписание звонков."""
    res = await _qc_require_user(update, context)
    if not res:
        return
    user, class_obj = res
    if not class_obj:
        await update.message.reply_text("Вы не состоите в классе.")
        return
    await update.message.reply_text(get_bells_info(class_obj, user))


async def qc_weather(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/weather, /погода — погода сейчас."""
    res = await _qc_require_user(update, context)
    if not res:
        return
    ...
    if not getattr(user, 'city', None):
        await update.message.reply_text(
            "🏙 Город не указан. Установите его в ⚙️ Настройки → 🌦 Настройки погоды."
        )
        return
...
async def qc_timer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/timer, /таймер — быстрый таймер (инлайн-кнопки)."""
    res = await _qc_require_user(update, context)
    if not res:
        return
    user, class_obj = res
...
async def qc_stars(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/stars, /звезды — меню звёзд."""
    res = await _qc_require_user(update, context)
    if not res:
        return
...
async def qc_auto(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/auto, /авто — войти в режим автоматизации (DeepSeek)."""
    user_id = str(update.effective_user.id)
    user = get_user(user_id)
    if not user or not user.setup_completed:
        await update.message.reply_text(
    ...
    if not await ensure_subscribed(update, context):
        return
    if context.user_data.get('auto_mode'):
        context.user_data['auto_mode'] = False
        context.user_data.pop('pending_auto_intent', None)
        await update.message.reply_text(
            "👋 Вы вышли из режима автоматизации.",
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif data == "buy_stars_custom":
        # ПУНКТ 1 (динамические чеки): своя сумма покупки звёзд.
        # Важно: эта проверка стоит ВЫШЕ startswith("buy_stars_"),
        # иначе callback был бы ошибочно обработан как фиксированный пакет.
        return await buy_stars_custom_start(update, context)
    ...
    # === ПУНКТ 1 (платные кнопки-товары): оплата кастомных товаров ===
    elif data.startswith("prod_pay_virt_"):
        return await prod_pay_virt_handler(update, context)
    elif data.startswith("prod_pay_xtr_"):
        return await prod_pay_xtr_handler(update, context)

    # === ⚡ АВТОМАТИЗАЦИЯ: подтверждение выполнения распознанных команд ===
    elif data == "auto_exec_yes":
        return await auto_exec_yes_handler(update, context)
    elif data == "auto_exec_no":
        return await auto_exec_no_handler(update, context)
    elif data == "auto_examples":
        return await auto_examples_callback_handler(update, context)
    elif data == "auto_exit":
        return await auto_exit_callback_handler(update, context)
...
async def cancel_action_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    temp_keys = [
        ...
        # Платные кнопки: незавершённое создание товара.
        'custom_button_content', 'global_button_content',
    ]
...
async def detect_timezone_for_city(city):
    """ПУНКТ «Погода» (часовой пояс): вернуть смещение пользователя
    относительно UTC (в часах) для указанного города.
    ...
    ИСПРАВЛЕНО: раньше смещение округлялось до ЦЕЛОГО часа, из-за чего в
    зонах вида UTC+5:30 (Индия) или UTC+9:30 (Аделаида) все уведомления
    сдвигались на полчаса. Теперь точность — четверть часа, и значение
    может быть дробным (например 5.5).
    """
    ...
    if data and "location" in data:
        ...
        if local_str:
            try:
                ...
                # Округляем до четверти часа (поддержка UTC+5:30 и т. п.),
                # ограничиваем разумным диапазоном.
                tz_offset = round(diff_hours * 4) / 4
                if -12 <= tz_offset <= 14:
                    return int(tz_offset) if float(tz_offset).is_integer() else tz_offset
            except Exception as e:
                logger.warning(
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    if resp.status == 200:
                        ...
                        # dst_offset — секунды DST, нам нужна сумма.
                        raw = int(wt.get("raw_offset", 0) or 0)
                        dst = int(wt.get("dst_offset", 0) or 0)
                        # Точность — четверть часа (900 секунд).
                        tz_offset = round((raw + dst) / 900.0) / 4
                        if -12 <= tz_offset <= 14:
                            return int(tz_offset) if float(tz_offset).is_integer() else tz_offset
        except Exception as e:
...
async def enter_city_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    sign_offset = format_tz_offset(tz_offset)
    await update.message.reply_text(
        f"✅ Город установлен: {canonical}\n"
        f"🕒 Часовой пояс определён автоматически: {sign_offset}\n\n"
        f"Теперь в заданное время я буду присылать вам погоду и советы по "
        f"одежде, а также уведомления о праздниках и пожелания. "
...
async def weather_settings_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    weather_time = getattr(user, 'weather_notification_time', '07:00')
    city = getattr(user, 'city', None) or "не выбран"
    tz_str = format_tz_offset(getattr(user, 'timezone', 3))
    # ИСПРАВЛЕНО (погода): раньше здесь всегда писалось «рассылка ОТКЛЮЧЕНА»,
    # хотя теперь ежедневная погода снова приходит по расписанию.
    status = "✅ Включена" if enabled else "❌ Выключена"
    text = (
        f"🌦 **Настройки погоды**\n\n"
        f"{status} — ежедневная авто-рассылка погоды.\n"
        f"⏰ Время: {weather_time} (ваше местное, пояс {tz_str})\n"
        f"🏙 Город: {city}\n\n"
        f"Погода приходит один раз в день в указанное время.\n"
        f"Расписание хранится в базе и переживает перезапуск бота.\n\n"
        f"Выберите действие:"
    )
    try:
        await query.edit_message_text(
...
async def change_city_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"🕒 Часовой пояс пересчитан: {format_tz_offset(tz_offset)}",
        reply_markup=get_settings_keyboard(user)
    )
    return USER_SETTINGS
...
# In-memory cooldown для ежедневной погоды: uid → monotonic-время последней
# попытки. Защищает WeatherAPI от долбёжки при повторных ошибках отправки
# (максимум одна попытка на пользователя раз в 15 минут).
_WEATHER_LAST_ATTEMPT = {}
...
def _user_local_now(user):
    ...
    защищает утренние/вечерные уведомления от дрейфа системных часов
    хостинга.

    ИСПРАВЛЕНО (часовой пояс): теперь поддерживаются ДРОБНЫЕ пояса
    (UTC+5:30 и т. п.) — parse_tz_hours корректно парсит int, float и
    строки «+3»/«UTC+3»/«UTC+5:30», не обрезая полчаса до целого.
    """
    tz_raw = getattr(user, 'timezone', 3) if user else 3
    tz_hours = parse_tz_hours(tz_raw, default=3.0)
    return _now_utc() + timedelta(hours=tz_hours)
...
async def _unified_notification_tick_locked(context):
    ...
    for uid, user in users.items():
        try:
            ...
            # 2c) Погода — ежедневная авто-рассылка.
            #
            # ИСПРАВЛЕНО (баг «Погода»): раньше этот блок был намеренно
            # отключён (слот сразу помечался «отправленным», ничего не
            # приходило). Теперь погода снова приходит каждый день в
            # user.weather_notification_time по ЛОКАЛЬНОМУ времени
            # пользователя (с учётом его часового пояса, включая дробные
            # UTC+5:30).
            #
            # ПУНКТ «сохранение задач рассылки в БД»: расписание живёт в
            # users.json (user.weather_notifications / weather_notification_time),
            # а журнал отправок — в notification_log.json. Оба файла —
            # персистентные (Supabase/MongoDB/локальный JSON), поэтому
            # перезапуск бота НЕ теряет задачи рассылки: после старта тикер
            # дочитает расписание из БД и корректно продолжит отправки —
            # благодаря окну догона пропущенное уведомление даже придёт
            # с опозданием, а не потеряется.
            if getattr(user, 'weather_notifications', True) and getattr(user, 'city', None):
                ...
                due, too_late, w_occ = _is_daily_time_due(
                    local_now, weather_t, max_late_minutes=6 * 60
                )
                if w_occ and not _notification_already_sent(log, uid, 'weather', w_occ):
                    if due:
                        # Защита от долбёжки API: не чаще одной попытки
                        # на пользователя раз в 15 минут.
                        now_monotonic = time.monotonic()
                        last_try = _WEATHER_LAST_ATTEMPT.get(uid, 0.0)
                        if now_monotonic - last_try < 900:
                            pass  # пропускаем этот тик, попробуем позже
                        else:
                            _WEATHER_LAST_ATTEMPT[uid] = now_monotonic
                            try:
...
async def _post_init(application):
    ...
    try:
        ...
        for diag_uid, diag_user in users_for_diag.items():
            tz = parse_tz_hours(getattr(diag_user, 'timezone', 3))
            # ВАЖНО: используем `_now_utc()` — это «реальное» UTC, скорректированное
...
def main():
    ...
    # ПУНКТ 3 (статистика Stars): при старте полностью очищаем подробную
    # историю переводов из stars_stats.json и пересчитываем агрегаты —
    # общее число пользователей, суммарно потраченные звёзды и топ трат.
    try:
        _update_stars_aggregates()
    except Exception as e:
        logger.error(f"Не удалось пересчитать статистику звёзд: {e}")
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            # ИСПРАВЛЕНО (бесконечный цикл звонков): теперь существует
            # отдельное состояние для ввода ВРЕМЕНИ ОКОНЧАНИЯ урока —
            # ранее save_bell_end_handler не был зарегистрирован вообще,
            # и бот бесконечно спрашивал «введите время начала».
            EDIT_BELL_END: [
            ...
            # === НОВЫЕ состояния (динамические чеки + платные кнопки-товары) ===
            BUY_STARS_CUSTOM: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, buy_stars_custom_amount_handler),
                CallbackQueryHandler(handle_callback),
            ],
            CUSTOM_BUTTON_PRICE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, custom_button_price_handler),
                CallbackQueryHandler(handle_callback),
            ],
            DEV_GLOBAL_BUTTON_PRICE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, dev_global_button_price_handler),
                CallbackQueryHandler(handle_callback),
            ],
        },
        fallbacks=[
    ...
    # ==============================================================
    # === ГРУППА -1: БЫСТРЕЕ FSM — работают из любого состояния ===
    # ==============================================================
    # Обработчики группы -1 выполняются ДО ConversationHandler (группа 0),
    # поэтому команды срабатывают в любом месте бота: во время ввода ДЗ,
    # в админ-панели, в чате с AI и т. д. Состояние FSM не меняется —
    # после команды пользователь остаётся там же, где был.
    # /start, /restore и /cancel сюда НЕ входят — их по-прежнему обрабатывает
    # основная машина состояний.

    # ⌨️ Быстрые команды. Латинские — через CommandHandler, кириллические
    # (/дз, /учителя …) — через regex-диспетчер: PTB разрешает регистрировать
    # через CommandHandler только команды [a-z0-9_], а Telegram-клиенты
    # при этом отлично отправляют команды кириллицей.
    application.add_handler(CommandHandler(["dz", "homework", "hw"], qc_homework), group=-1)
    application.add_handler(CommandHandler(["teachers", "teachers_list"], qc_teachers), group=-1)
    application.add_handler(CommandHandler(["schedule"], qc_schedule), group=-1)
    application.add_handler(CommandHandler(["today"], qc_today), group=-1)
    application.add_handler(CommandHandler(["tomorrow"], qc_tomorrow), group=-1)
    application.add_handler(CommandHandler(["bells"], qc_bells), group=-1)
    application.add_handler(CommandHandler(["weather"], qc_weather), group=-1)
    application.add_handler(CommandHandler(["timer"], qc_timer), group=-1)
    application.add_handler(CommandHandler(["stars"], qc_stars), group=-1)
    application.add_handler(CommandHandler(["auto", "automation"], qc_auto), group=-1)
    # Кириллические алиасы: /дз /учителя /расписание /сегодня /завтра
    # /звонки /погода /таймер /звезды /авто (и вариант с @имя_бота).
    application.add_handler(
        MessageHandler(filters.Regex(_QUICK_CMD_REGEX), quick_command_dispatcher),
        group=-1,
    )

    # ⚡ Оверлей автоматизации: пока включён auto_mode, ЛЮБОЙ текст
    # (не команда) уходит в парсер DeepSeek, а не в текущее состояние FSM.
    application.add_handler(
        MessageHandler(filters.TEXT & ~filters.COMMAND, automation_gate_handler),
        group=-1,
    )
    # ==============================================================
