...
# ==================================
# === ХРАНИЛИЩЕ: КОНФИГ КАНАЛА ===
# ==================================
# storage_config.json: { channel_id, channel_title, auto_backup,
...
STORAGE_CONFIG_DEFAULTS = {
    "channel_id": None,        # int: -100…
    "channel_title": "",       # человекочитаемое имя канала
    "auto_backup": True,       # ежедневный авто-бэкап базы в канал
    "backup_hour_utc": 4,      # час UTC, в который делаем бэкап
    "last_backup_date": None,  # "YYYY-MM-DD" (UTC) — защита от дублей
    "backups": [],             # реестр: [{date, ts, message_id, file_id, size, users, classes, files}]
...
def get_storage_channel_id():
    """ID канала-хранилища: из конфига (UI) или из env STORAGE_CHANNEL_ID.

    Возвращает int или None. @username из env здесь НЕ резолвим (нужен
    сетевой вызов) — env принимаем только числовым."""
...
async def _voice_transcription_middleware(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # === Облако (режим загрузки файлов): аудио и голосовые НЕ транскрибируются —
        # пользователь сохраняет сам файл. Флаг self-heals: любой текстовый апдейт
        # выводит пользователя из режима загрузки, STT снова включается.
        if att is not None and context.user_data.get('cloud_file_mode'):
            return
        if getattr(msg, "text", None) and context.user_data.get('cloud_file_mode'):
...
async def _storage_upload_document(context, data: bytes, filename: str, caption: str = ""):
    """Загружает документ в приватный канал. Возвращает
    {"message_id", "file_id", "size"} или None (ошибка логируется)."""
    ...
    try:
        ...
        logger.error(f"storage: загрузка документа в канал не удалась: {e}")
        return None


async def _storage_do_backup(context):
    """Собирает ВСЕ файлы данных в zip и грузит в приватный канал.

    Возвращает (ok, человекочитаемый отчёт). Реестр бэкапов хранится в
    storage_config.json (Bot API не умеет читать историю канала, поэтому
    указатели — message_id/file_id — сохраняем сами)."""
    channel_id = get_storage_channel_id()
    if not channel_id:
        return False, ("❌ Хранилище не настроено. Подключите приватный канал: "
                       "🛠️ Панель разработчика → ☁️ Хранилище → 🔗 Задать канал.")
    files = {}
    ...
    try:
        sent = await context.bot.send_document(
            chat_id=channel_id,
            document=InputFile(payload, filename=f"devorks_backup_{today}.zip"),
            caption=caption[:1024],
        )
    except Exception as e:
        return False, f"❌ Бэкап не загрузился в канал: {e}"
    entry = {
        "date": today,
...
async def cloud_help_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        "❓ Как работает облако\n\n"
        "1. Разработчик один раз подключает приватный Telegram-канал "
        "(🛠️ Панель разработчика → ☁️ Хранилище) и делает бота его админом.\n"
        "2. Вы отправляете боту файл (документ/фото/видео/аудио) — бот кладёт его "
        "в этот канал. Telegram хранит файлы бесплатно и бессрочно.\n"
        "3. Список ваших файлов — «📁 Мои файлы». По кнопке 📥 бот мгновенно "
        "присылает файл обратно в любой чат с вами.\n\n"
        "Честные ограничения:\n"
        "• до 49 МБ на один файл (лимит Telegram Bot API);\n"
        "• лимит файлов на человека задаёт разработчик (по умолчанию 50);\n"
        "• бот не может читать историю канала — поэтому «показать ВСЁ содержимое "
        "канала» он не умеет, только файлы, которые загрузил через себя."
    )
    try:
...
async def cloud_upload_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if fsize > STORAGE_MAX_FILE_BYTES:
        await msg.reply_text(
            f"🚫 Файл слишком большой ({_fmt_bytes(fsize)}). Лимит Telegram для "
            "ботов — 50 МБ на один файл. Сожмите файл или разбейте на части."
        )
        return CLOUD_UPLOAD_WAIT
...
def get_storage_dev_keyboard(cfg=None):
    ...
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔗 Задать канал", callback_data="dev_storage_set")],
...
def _storage_dev_status_text(cfg):
    ...
    lines = ["☁️ Хранилище (приватный канал)", ""]
    if ch:
        title = cfg.get("channel_title") or "канал"
        lines.append(f"🔗 Канал: «{title}» (id {ch})")
    else:
        lines.append("❌ Канал не задан.")
    ...
    try:
        ...
        lines.append(f"☁️ Файлов у пользователей: {total_files} • {_fmt_bytes(total_size)}")
    except Exception:
        pass
    lines.append("")
    lines.append(
        "Как подключить: создайте ПРИВАТНЫЙ канал → добавьте бота АДМИНОМ → "
        "нажмите «🔗 Задать канал» и перешлите сюда любое сообщение из этого "
        "канала (или отправьте -100…ID). Бот не может вступить по ссылке — только "
        "через добавление админом. Бот не читает историю канала: реестр бэкапов "
        "хранится в storage_config.json."
    )
    return "\n".join(lines)
...
async def dev_storage_set_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        "Бот проверит доступ тестовой загрузкой и сохранит канал.",
        reply_markup=InlineKeyboardMarkup([[
...
async def dev_storage_channel_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        return DEV_STORAGE_CHANNEL
    cfg = load_storage_config()
    cfg["channel_id"] = chat_id
    cfg["channel_title"] = title or str(chat_id)
    save_storage_config(cfg)
    await msg.reply_text(
        f"✅ Хранилище подключено: «{cfg['channel_title']}» (id {chat_id}).\n\n"
        "Теперь доступны: личное облако (кнопка «☁️ Облако») и авто-бэкап базы "
        "раз в день. Управление: 🛠️ Панель разработчика → ☁️ Хранилище.",
        reply_markup=get_main_menu_keyboard(get_user(DEVELOPER_ID) or User(DEVELOPER_ID)),
    )
    return DEV_PANEL
...
def _automation_class_context_text(user, class_obj):
    ...
    # Хранилище/облако — контекст для действий show_storage/backup_now.
    try:
        _st_ch = get_storage_channel_id()
        _st_files = [f for f in (getattr(user, 'cloud_files', []) or []) if isinstance(f, dict)]
        _st_limit = get_price('cloud_max_files', 50)
        if _st_ch:
            lines.append(
                f"Хранилище: приватный канал подключён; файлов в облаке пользователя: "
                f"{len(_st_files)} из {_st_limit}."
            )
        else:
            lines.append(
                "Хранилище: приватный канал НЕ подключён — облако и бэкапы "
...
async def _automation_execute_action(update, context, user, class_obj, action):
    ...
    if name == "show_storage":
        ...
        lines = []
        if channel_id:
            lines.append("☁️ Хранилище (приватный канал): подключено.")
        else:
            lines.append(
