...
async def handle_ai_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Если пришло фото — скачиваем самое крупное; сначала пробуем ЛОКАЛЬНЫЙ
    # OCR (бесплатно и безлимитно), при неудаче — вижн-модель Groq.
    photo_data_url = None
    ocr_text = None
    photo_bytes = None
    if has_photo:
...
def _automation_system_prompt(context_text, is_admin):
    ...
    return (
        ...
        '9) {"action":"create_timer","date":"ГГГГ-ММ-ДД","time":"ЧЧ:ММ","text":"<текст напоминания>"} — таймер/напоминание (доступно всем). Если пользователь говорит «через N минут/часов» — используй вместо даты поле in_minutes: {"action":"create_timer","in_minutes":<целое число минут>,"text":"<текст>"}. Разрешено передавать date как «today»/«tomorrow» — исполнитель сам посчитает дату.\n'
